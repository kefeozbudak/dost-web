...
if (!page_editor.includes('const [activeArrayItem, setActiveArrayItem] = useState')) {
...
if (page_editor.includes(old_handleBlockClick)) {
    ...
    console.log("WARNING: handleBlockClick not found in PageEditor.tsx");
}
...
if (!block_editor.includes('activeArrayItem?: { arrayKey: string, index: number } | null')) {
...
const old_array_div = `      {(block[arrayKey] || []).map((item: any, idx: number) => (
        <div 
          key={idx} 
          draggable
...
const new_array_div = `      {(block[arrayKey] || []).map((item: any, idx: number) => (
        <details 
          key={idx} 
...
if (block_editor.includes(old_array_div)) {
    block_editor = block_editor.replace(old_array_div, new_array_div);
    
    const old_array_close = `          <button onClick={() => {
    ...
    const new_array_close = `          <button onClick={() => {
            const newItems = [...(block[arrayKey] || [])];
            newItems.splice(idx, 1);
            handleChange(arrayKey, newItems);
