...
# Add import
target_import = r"import \{ Trash2, Plus, GripVertical, Image as ImageIcon \} from 'lucide-react';"
replacement_import = r"import { Trash2, Plus, GripVertical, Image as ImageIcon } from 'lucide-react';\nimport MediaPickerModal from '../components/MediaPickerModal';"
...
content = re.sub(target_state, "", content) # Remove the one added incorrectly before if any

target_state_clean = r"export default function BlockFormEditor\(\{ block, onChange, pagesList \}: BlockFormEditorProps\) \{\n  if \(\!block\) return <div className=\"text-sm text-slate-500 text-center py-8\">Lütfen düzenlemek için bir modül seçin\.<\/div>;"
replacement_state_clean = r"""export default function BlockFormEditor({ block, onChange, pagesList }: BlockFormEditorProps) {
  const [mediaPickerConfig, setMediaPickerConfig] = useState<{ isOpen: boolean; onSelect: (url: string) => void }>({ isOpen: false, onSelect: () => {} });
  if (!block) return <div className="text-sm text-slate-500 text-center py-8">Lütfen düzenlemek için bir modül seçin.</div>;"""
content = re.sub(target_state_clean, replacement_state_clean, content)
