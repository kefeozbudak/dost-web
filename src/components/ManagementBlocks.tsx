    ...
    <div className={`mb-16 w-full ${block.styles?.fullWidth ? "max-w-full px-6" : "max-w-[1200px] px-6"} mx-auto whitespace-pre-line`} style={getStyle(block)}>
      ...
      <div className={`grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4`}>
        {items.map((item: any, idx: number) => {
          if (isDeans) {
            return (
              <div key={idx} className={getCardClass(item, "rounded-xl overflow-hidden bg-white shadow hover:shadow-lg transition-shadow border border-slate-100 flex flex-col")} style={getCardStyle(item, block)}>
                {item.image ? (
                  <div 
