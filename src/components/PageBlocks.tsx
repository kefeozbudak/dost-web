...
const ClubsGridBlock = ({
  ...
  return (
    <div key={block.id || "clubs-grid"} style={getStyle(block, "container")}>
      <section
        className="py-12 bg-white border-b border-border-subtle sticky top-20 z-40"
        id="clubs"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredItems.map((item: any, i: number) => {
            ...
            return (
              <div
                key={i}
                className="bg-surface-card rounded-xl border border-border-subtle overflow-hidden flex flex-col hover:-translate-y-1 hover:shadow-lg transition-all duration-300 group"
              ...
              >
                ...
                <div className="p-6 flex-grow flex flex-col">
                  ...
                  <p
                    className="text-on-surface-variant font-body-md mb-6 line-clamp-3"
...
export const DynamicBlockRenderer = ({
  ...
  const getStyle = (block: any, prefix: string) => {
    return {
      ...
      marginBottom: block.styles?.[prefix + "MarginBottom"]
        ...
        : undefined,
      backgroundColor:
        (prefix === "" ? block.styles?.backgroundColor : undefined) ||
  ...
  const renderBlock = (block: any, index: number) => {
    const renderContent = () => {
      ...
      case 'campus_hero':
        return (
          <section key={index} className="relative h-[600px] w-full flex items-center overflow-hidden transition-all duration-1000 opacity-100 translate-y-0" style={getStyle(block, 'container')}>
            <div className="absolute inset-0 z-0">
              <div className="absolute inset-0 bg-gradient-to-r from-primary/80 to-transparent z-10"></div>
              {block.image && <img className="w-full h-full object-cover" src={block.image} alt="" style={{...getImageStyle(block, 'image'), backgroundSize: 'cover', backgroundPosition: 'center center'}} />}
            </div>
            <div className="relative z-20 max-w-container-max mx-auto px-margin-desktop w-full">
              <div className="max-w-2xl">
                <h1 className="font-display-lg text-display-lg text-white mb-6" style={getTitleStyle(block)}>{block.title}</h1>
                <p className="font-body-md text-body-lg text-white/90 mb-8 leading-relaxed" style={getSubtitleStyle(block)}>
                  {block.subtitle}
                </p>
      ...
      case 'campus_bento':
        ...
        return (
          <section key={index} className="py-section-gap max-w-container-max mx-auto px-margin-desktop transition-all duration-1000 opacity-100 translate-y-0" style={getStyle(block, 'container')}>
            <div className="text-center mb-16">
              <h2 className="font-headline-xl text-headline-xl text-text-main mb-4" style={getTitleStyle(block)}>{block.title}</h2>
              ...
              {(block.items || []).map((item: any, i: number) => {
                ...
                return (
                  <div key={i} className={`${cols} group relative overflow-hidden rounded-3xl h-[400px] border border-border-subtle hover:border-primary transition-all duration-500 shadow-sm`} style={baseStyle}>
                    {item.image && (
                      <img className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" src={item.image} alt="" style={getImageStyle(item, 'image', i)} />
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-8">
                      <h3 className="font-headline-md text-headline-md text-white mb-2" style={getCardTitleStyle(item, block)}>{item.title}</h3>
                      {item.desc && <p className="text-white/80 font-body-md" style={getCardDescStyle(item, block)}>{item.desc}</p>}
                    </div>
                  </div>
                );
              })}
      ...
      case 'campus_gallery':
        return (
          <section key={index} className="bg-surface-container-low py-section-gap transition-all duration-1000 opacity-100 translate-y-0" style={getStyle(block, 'container')}>
            <div className="max-w-container-max mx-auto px-margin-desktop">
              <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                <div>
                  <h2 className="font-headline-xl text-headline-xl text-text-main mb-4" style={getTitleStyle(block)}>{block.title}</h2>
                  <p className="font-body-md text-text-muted max-w-xl" style={getSubtitleStyle(block)}>{block.subtitle}</p>
                </div>
              ...
              <div className="flex gap-6 overflow-x-auto no-scrollbar pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                {(block.items || []).map((item: any, i: number) => (
                  <div key={i} className="min-w-[350px] bg-white rounded-2xl overflow-hidden shadow-sm border border-border-subtle group flex-shrink-0" style={getCardStyle(item)}>
                    <div className="h-64 overflow-hidden relative">
                      {item.image && <img className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src={item.image} alt="" style={getImageStyle(item, 'image', i)} />}
                    </div>
                    <div className="p-6">
                      <h4 className="font-headline-md text-[20px] mb-2" style={getCardTitleStyle(item, block)}>{item.title}</h4>
                      <p className="text-text-muted text-label-md" style={getCardDescStyle(item, block)}>{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="grid grid-cols-2 gap-6 items-center">
                  <div className="flex flex-col gap-6">
                    <div className="h-48 rounded-2xl overflow-hidden shadow-lg ">
                      {block.image1 && <img className="w-full h-full object-cover" src={block.image1} alt="" />}
                    </div>
                    <div className="h-64 rounded-2xl overflow-hidden shadow-lg">
                      {block.image2 && <img className="w-full h-full object-cover" src={block.image2} alt="" />}
                    </div>
                  </div>
                  <div className="flex flex-col gap-6 pt-12">
                    <div className="h-64 rounded-2xl overflow-hidden shadow-lg">
                      {block.image3 && <img className="w-full h-full object-cover" src={block.image3} alt="" />}
                    </div>
                    <div className="h-48 rounded-2xl overflow-hidden shadow-lg ">
                      {block.image4 && <img className="w-full h-full object-cover" src={block.image4} alt="" />}
                    </div>
                  </div>
                </div>
              </div>
              <div className="order-1 lg:order-2 space-y-8">
                {block.badge && <span className="text-primary font-bold text-label-md tracking-widest uppercase" style={getBadgeStyle(block)}>{block.badge}</span>}
                <h2 className="font-headline-xl text-headline-xl text-text-main" style={getTitleStyle(block)}>
                  {block.titlePart1} {block.titlePart2 && <div style={getTitlePart2Style(block)}>{block.titlePart2}</div>}
                </h2>
                <p className="font-body-md text-text-muted leading-relaxed" style={getSubtitleStyle(block)}>
                  {block.subtitle}
                </p>
      ...
      case 'campus_contact':
        return (
          <section key={index} className="py-section-gap bg-surface-dim/30 transition-all duration-1000 opacity-100 translate-y-0" style={getStyle(block, 'container')}>
            <div className="max-w-container-max mx-auto px-margin-desktop">
              <div className="bg-white rounded-[32px] overflow-hidden shadow-2xl flex flex-col lg:flex-row">
                <div className="p-12 lg:w-1/2 space-y-8 flex flex-col justify-center">
                  <h2 className="font-headline-xl text-headline-xl" style={getTitleStyle(block)}>{block.title}</h2>
                ...
                <div className="lg:w-1/2 min-h-[400px] relative overflow-hidden">
                  {block.mapCode ? (
                    <div
                      className="w-full h-full [&>iframe]:w-full [&>iframe]:h-full min-h-[400px]"
                      dangerouslySetInnerHTML={{ __html: block.mapCode }}
                    />
                  ) : (
                    <>
                      {block.image && <img className="absolute inset-0 w-full h-full object-cover" src={block.image} alt="" style={getImageStyle(block, 'image')} />}
                      <div className="absolute bottom-8 left-8 right-8 bg-white/80 backdrop-blur-md p-6 rounded-2xl flex items-center justify-between border border-border-subtle/80">
                        <div>
                          <h4 className="font-bold">{block.cardTitle}</h4>
          ...
          return (
            ...
            >
              <div className="max-w-container-max mx-auto px-margin-desktop text-center">
                ...
                <p
                  className="font-body-lg text-body-lg text-text-muted max-w-2xl mx-auto"
        ...
        case "contact_campuses":
          return (
            <section
              key={index}
              className="py-section-gap bg-surface-background"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop">
                <div className="flex flex-col gap-12">
                  {(block.items || []).map((item: any, i: number) => (
                    <div
                      key={i}
                      className="grid grid-cols-1 md:grid-cols-3 gap-0 bg-white rounded-2xl border border-border-subtle overflow-hidden shadow-sm"
                      style={getCardStyle(item)}
                    >
                      <div className="md:col-span-1 p-8 flex flex-col justify-center border-r border-border-subtle">
                        {item.badge && (
                          <div className="bg-primary text-on-primary px-3 py-1 rounded text-caption font-caption w-fit mb-4 uppercase">
                            {item.badge}
                          </div>
                        )}
                        <h3
                          className="font-headline-xl text-headline-xl text-on-surface mb-4"
                          style={getCardTitleStyle(item, block)}
                        >
                          {item.title}
                        </h3>
                      ...
                      <div className="md:col-span-2 h-[400px] relative bg-surface-container overflow-hidden">
                        {item.mapCode ? (
                          <div
                          ...
                          <>
                            <div
                              className="absolute inset-0 grayscale-[0.5] opacity-80"
                              style={{
                            ...
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              <div className="bg-white/90 backdrop-blur-sm px-6 py-3 rounded-lg shadow-lg border border-primary/20 pointer-events-auto">
        ...
        case "contact_form":
          return (
            <section
              key={index}
              className="py-section-gap bg-surface-container-low"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop">
                <div className="bg-white p-8 md:p-12 rounded-2xl shadow-sm border border-border-subtle">
                  <div className="mb-8">
                    <h2
                    ...
                    {block.subtitle && (
                      <p
                        className="font-body-md text-body-md text-text-muted"
                        style={getSubtitleStyle(block)}
                      >
                        {block.subtitle}
                      </p>
                  ...
                  >
                    <div className="flex flex-col gap-2">
                      ...
                      <input
                        className="w-full p-3 rounded-lg border border-outline-variant font-body-md focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
                    ...
                    <div className="flex flex-col gap-2 md:col-span-2">
                      ...
                      <div className="relative">
                        <select className="w-full p-3 rounded-lg border border-outline-variant font-body-md focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all appearance-none bg-white">
                    ...
                    <div className="flex flex-col gap-2 md:col-span-2">
                      ...
                      <textarea
                        className="w-full p-3 rounded-lg border border-outline-variant font-body-md focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all resize-none"
                    ...
                    <div className="md:col-span-2">
                      <button
                        className="bg-primary text-on-primary px-8 py-3 rounded-lg font-label-md text-label-md hover:opacity-90 transition-all active:scale-95 flex items-center gap-2 w-fit"
                        type="submit"
                      >
                        Gönder{" "}
        ...
        case "social_media":
          return (
            <section
              key={index}
              className="py-12 bg-white border-t border-border-subtle"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop text-center">
                <h3
                  ...
                  style={getTitleStyle(block)}
                >
                  {block.title || "Sosyal Medyada Biz"}
                </h3>
                {block.subtitle && (
                  <p
                    className="font-body-md text-body-md text-text-muted mb-8 max-w-2xl mx-auto"
                    style={getSubtitleStyle(block)}
                  >
                    {block.subtitle}
                  </p>
                ...
                <div className="flex justify-center gap-6">
                  {(block.items || []).map((item: any, i: number) => (
                    <a
                      key={i}
                      href={item.url || "#"}
                      className="w-14 h-14 rounded-xl border border-border-subtle flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary transition-all group"
                    >
                      {item.icon &&
                        (typeof item.icon === "string" &&
                        item.icon === item.icon.toLowerCase() ? (
                          ...
                          <IconPreview
                            data={item.icon}
                            className="w-6 h-6 fill-current group-hover:fill-white transition-colors"
                          />
                        ))}
                    </a>
                  ))}
          ...
          return (
            ...
            >
              ...
              <div className="relative z-10 text-center px-gutter max-w-4xl">
                <h1
                  className="font-display-lg text-display-lg text-on-primary mb-6 drop-shadow-lg"
                  style={getTitleStyle(block)}
                >
                ...
                {block.subtitle && (
                  <p
                    className="font-body-lg text-body-lg text-on-primary-container opacity-90 max-w-2xl mx-auto mb-8"
                    style={getSubtitleStyle(block)}
                  >
                    {block.subtitle}
                  </p>
          ...
          return (
            ...
            >
              <div className="max-w-container-max mx-auto px-gutter">
                <div className="text-center mb-16">
                  <h2
                    className="font-headline-xl text-headline-xl text-primary mb-4"
                    style={getTitleStyle(block)}
                  >
                  ...
                  {block.subtitle && (
                    <p
                      className="text-on-surface-variant max-w-2xl mx-auto font-body-lg"
                      style={getSubtitleStyle(block)}
                    >
                      {block.subtitle}
                    </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  {(block.items || []).map((item: any, i: number) => (
                    <div
                      key={i}
                      className="bg-white p-8 rounded-2xl border border-border-subtle text-center hover:-translate-y-1 transition-transform shadow-sm"
                      style={getCardStyle(item)}
                    >
                      <div className="w-16 h-16 bg-primary-fixed rounded-2xl flex items-center justify-center mx-auto mb-6">
                        {item.icon &&
                          (typeof item.icon === "string" &&
                          item.icon === item.icon.toLowerCase() ? (
                      ...
                      <h4
                        className="font-headline-md text-headline-md text-on-surface mb-3 text-lg"
                        style={getCardTitleStyle(item, block)}
                      >
                        {item.title}
                      </h4>
        ...
        case "clubs_cta":
          return (
            ...
            >
              <div className="max-w-container-max mx-auto px-gutter">
                <div className="bg-primary rounded-3xl p-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 shadow-md">
                  <div className="relative z-10 text-center md:text-left">
                    <h2
                      className="font-headline-xl text-headline-xl text-on-primary mb-4"
                      style={getTitleStyle(block)}
                    >
                    ...
                    {block.subtitle && (
                      <p
                        className="text-primary-fixed opacity-90 max-w-lg font-body-lg"
                        style={getSubtitleStyle(block)}
                      >
                        {block.subtitle}
                      </p>
                  ...
                  <div className="absolute -left-20 -top-20 w-60 h-60 bg-white/5 rounded-full blur-2xl"></div>
                </div>
              </div>
            </section>
          );
        ...
        case "news_grid":
          return (
            ...
            >
              <section
                className="bg-surface-container-lowest sticky top-20 z-40 border-b border-border-subtle"
              ...
              <section
                className="py-10 md:py-section-gap px-margin-mobile md:px-margin-desktop"
                style={getStyle(block, "")}
              >
                <div
                ...
                >
                  {block.items?.map((item: any, i: number) => (
                    <div
                      key={i}
                      className="bg-surface-card rounded-xl border border-border-subtle overflow-hidden hover:shadow-sm transition-all group flex flex-col"
                      style={{
                        ...
                        {item.title && (
                          <h3
                            style={{
                              ...
                              color:
                                item.itemTitleColor ||
                                getItemTitleStyle(block).color,
                            }}
                            className="font-headline-md text-xl md:text-headline-md mb-3 group-hover:text-primary transition-colors line-clamp-2"
                          >
                            {item.title}
                          </h3>
                        )}
                        {item.desc && (
                          <p
                            style={{
                              ...
                              color:
                                item.itemDescColor ||
                                getItemDescStyle(block).color,
                            }}
                            className="font-body-md text-sm md:text-body-md text-on-surface-variant mb-6 line-clamp-3"
                          >
                            {item.desc}
                          </p>
                        )}
                ...
                {block.showPagination && (
                  <div className="mt-12 md:mt-16 flex justify-center items-center gap-2">
                    ...
                    <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-border-subtle text-on-surface-variant hover:bg-surface-container-high transition-colors font-label-md">
                      2
                    </button>
                    <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-border-subtle text-on-surface-variant hover:bg-surface-container-high transition-colors font-label-md">
                      3
          ...
          return (
            ...
            >
              ...
              >
                ...
                <form
                  className="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto w-full"
                  onSubmit={(e) => e.preventDefault()}
                >
                  <input
                    className="flex-grow px-4 md:px-6 py-3 md:py-4 rounded-xl border border-border-subtle focus:ring-2 focus:ring-primary outline-none text-sm md:text-body-md"
                    placeholder={block.inputPlaceholder || "E-posta adresiniz"}
                    required
                    type="email"
                  />
                  <button
                    className="bg-primary text-on-primary px-6 md:px-8 py-3 md:py-4 rounded-xl font-label-md text-sm md:text-label-md hover:opacity-90 transition-all shadow-md"
                    type="submit"
                  >
            ...
            >
              <div className="w-full flex flex-col gap-6 md:gap-10">
                {(() => {
                  ...
                  const textContent = (
                    <div
                      className={`${block.fullWidth || block.styles?.fullWidth ? "max-w-container-max mx-auto" : "w-full"} text-center relative z-10`}
                    >
                      {block.badge && (
                        <span
                          style={getBadgeStyle(block)}
                          className="inline-block py-1.5 px-6 rounded-full bg-blue-100/80 text-blue-800 text-[11px] font-bold mb-4 tracking-widest"
                        >
                          {block.badge}
                        </span>
                      )}
                      ...
                      <p
                        style={getSubtitleStyle(block)}
                        className="text-base md:text-[17px] text-[#556987] max-w-3xl mx-auto whitespace-pre-line font-medium leading-relaxed"
                        dangerouslySetInnerHTML={{
                        ...
                        return (
                          <div className="max-w-container-max mx-auto w-full flex flex-col md:flex-row gap-3 md:gap-6 relative z-10 px-4 md:px-0 h-[560px] md:h-[480px]">
                            {itemsToRender.map((item: any, i: number) => (
                              <div
                                key={i}
                                className="group relative rounded-[24px] md:rounded-[32px] overflow-hidden min-h-[110px] md:min-h-0 h-full flex-1 hover:flex-[2.5] active:flex-[2.5] md:hover:flex-[2] lg:hover:flex-[3] transition-all duration-700 ease-in-out cursor-pointer select-none"
                              >
                                ...
                                <div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 w-max max-w-[90%] z-10">
                                  {item.url ? (
                                    <a
                                      ...
                                      className="bg-white/95 backdrop-blur-sm text-primary font-bold text-[13px] md:text-[15px] px-5 md:px-8 py-2 md:py-3 rounded-full shadow-lg whitespace-nowrap block text-center transition-transform group-hover:-translate-y-1 duration-300 hover:bg-primary hover:text-white cursor-pointer"
                                    >
                                      {item.title}
                                    </a>
                                  ) : (
                                    <span
                                      style={getItemTitleStyle(block)}
                                      className="bg-white/95 backdrop-blur-sm text-primary font-bold text-[13px] md:text-[15px] px-5 md:px-8 py-2 md:py-3 rounded-full shadow-lg whitespace-nowrap block text-center transition-transform group-hover:-translate-y-1 duration-300"
                                    >
                                      {item.title}
                                    </span>
                                  )}
              ...
              >
                <div className="flex flex-row items-center gap-4 mb-10 md:mb-12">
                  ...
                  <div>
                    ...
                    <h2
                      style={getTitleStyle(block)}
                      className="text-2xl md:text-3xl lg:text-4xl font-bold text-white whitespace-pre-line leading-tight"
                      dangerouslySetInnerHTML={{ __html: block.title || "" }}
                    ></h2>
                  </div>
                </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-6">
                  {block.items?.map((item: any, i: number) => (
                    <div
                      key={i}
                      className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group border border-border-subtle flex flex-col"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
                  ...
                  <div className="lg:col-span-5 flex flex-col justify-center">
                    {block.title && (
                      <h2
                        style={getTitleStyle(block)}
                        className="font-display-lg text-4xl md:text-5xl lg:text-display-lg text-on-surface mb-6 leading-tight whitespace-pre-line"
                        dangerouslySetInnerHTML={{ __html: block.title }}
                      ></h2>
                    )}
                    {block.subtitle && (
                    ...
                    {block.desc && (
                      <div
                        className="prose prose-slate max-w-none text-on-surface-variant"
                        dangerouslySetInnerHTML={{ __html: block.desc }}
                      ></div>
                    )}
                  </div>
        ...
        case "achievements_hero":
          return (
            <section
              key={index}
              className="relative bg-[#00164f] text-white overflow-hidden min-h-[600px] flex items-center"
              style={{
            ...
            >
              <div className="max-w-container-max mx-auto px-margin-desktop grid grid-cols-1 gap-6 relative z-10 py-20 items-center">
                <div className="space-y-6 text-center mx-auto">
                  {block.badge && (
                    <span className="inline-block bg-primary-container/20 text-inverse-primary px-4 py-1 rounded-full text-label-sm font-label-sm">
                      {block.badge}
                    </span>
                  )}
                  {block.titlePart1 || block.titlePart2 ? (
                    ...
                    <h1
                      className="font-display-lg text-display-lg leading-tight"
                      dangerouslySetInnerHTML={{ __html: block.title || "" }}
                    ></h1>
                  )}
                  {block.subtitle && (
                    <p
                      className="text-body-lg text-primary-fixed max-w-xl mx-auto"
                      dangerouslySetInnerHTML={{ __html: block.subtitle }}
                    ></p>
                  )}
                  {block.buttons && block.buttons.length > 0 && (
              ...
              <div className="max-w-container-max mx-auto px-margin-desktop">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  {block.items && block.items[0] && (
                    <div className="bg-surface-card border border-border-subtle rounded-3xl p-8 bento-card relative overflow-hidden transition-all duration-700 hover:-translate-y-2">
                      <div
                      ...
                      <div className="space-y-6">
                        <div className="flex items-end gap-3">
                          ...
                          <span className="text-label-md text-on-surface-variant pb-1">
                            {block.items[0].statLabel}
                          </span>
                        </div>
                        <div className="h-2 w-full bg-surface-container rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gold w-[98%]"
                            style={{ backgroundColor: "#D4AF37" }}
                          ></div>
                        </div>
                        <p className="text-label-sm font-label-sm text-primary uppercase">
                          {block.items[0].tag}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-primary text-on-primary rounded-3xl p-8 bento-card relative overflow-hidden transition-all duration-700 hover:-translate-y-2">
                      ...
                      {block.items[1].buttonText && (
                        <a
                          ...
                          className="mt-8 block text-center w-full py-3 bg-gold text-[#00164f] font-bold rounded-xl hover:opacity-90 transition-opacity"
                          style={{ backgroundColor: "#D4AF37" }}
                        >
                          {block.items[1].buttonText}
                        </a>
                        ...
                        {block.items[2].list?.map(
                          (listItem: string, i: number) => (
                            <li key={i} className="flex items-center gap-3">
                              ...
                              <span className="text-label-md">{listItem}</span>
                            </li>
                          ),
                        )}
                      </ul>
        ...
        case "achievements_grid":
          return (
            <section
              key={index}
              className="py-section-gap bg-white"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop">
        ...
        case "achievements_science":
          return (
            <section
              key={index}
              className="py-section-gap bg-surface-container-low overflow-hidden"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop">
                <div className="bg-[#0f172a] rounded-[40px] p-12 md:p-20 relative overflow-hidden text-white">
                  <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/10 rounded-full blur-[100px] -mr-[250px] -mt-[250px]"></div>
                  <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                    <div>
                      {block.badge && (
                        <span
                          className="inline-block bg-gold/20 px-4 py-1 rounded-full text-label-sm font-label-sm mb-6"
                          style={{
                            color: "#D4AF37",
                    ...
                    <div className="relative">
                      <div className="rounded-3xl shadow-2xl border border-white/10 w-full aspect-video overflow-hidden relative">
                        <div
                          className="w-full h-full bg-cover bg-center"
                          style={getImageStyle(block, "image")}
                        ></div>
                      ...
                      {block.highlightTag && (
                        <div
                          className="absolute -top-4 -right-4 bg-gold text-[#00164f] p-4 rounded-xl font-bold shadow-lg"
                          style={{ backgroundColor: "#D4AF37" }}
                        >
                          {block.highlightTag}
                        </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mb-8 md:mb-12">
                    {block.items?.map((item: any, i: number) => (
                      <div key={i}>
                        <div
                          style={getItemTitleStyle(block)}
                          className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-secondary-fixed mb-1 md:mb-2"
                        >
                          {item.value}
                        </div>
                        <div
                          style={getItemDescStyle(block)}
                          className="text-white/70 text-xs md:text-sm font-bold tracking-wider"
                        >
                          {item.label}
                        </div>
                      </div>
              ...
              <div className="max-w-container-max mx-auto px-margin-desktop relative z-20 w-full">
                <div className="max-w-2xl text-white">
                  {block.badge && (
                    <span
                      style={getBadgeStyle(block)}
                      className="inline-block bg-secondary-container text-on-secondary-container px-4 py-1 rounded-full font-label-sm mb-6 uppercase tracking-wider"
                    >
                      {block.badge}
                    </span>
                  )}
                  ...
                  {block.items?.map((item: any, i: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className="w-full h-64 md:h-96 rounded-3xl shadow-lg border border-border-subtle relative overflow-hidden">
                            <div
                              className="absolute inset-0 w-full h-full bg-cover bg-center"
                              style={getImageStyle(item, "image")}
                            ></div>
        ...
        case "mission_vision":
          return (
            <section
              ...
              style={getStyle(block, "")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop grid md:grid-cols-2 gap-6">
                {block.items?.map((item: any, i: number) => {
                  const isSecondary = i % 2 !== 0;
                  return (
                    <div
                      key={i}
                      className={`bg-surface-card p-10 rounded-2xl border border-border-subtle shadow-sm flex flex-col items-center text-center group transition-colors duration-300 ${isSecondary ? "hover:border-secondary" : "hover:border-primary"}`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {block.items?.map((item: any, i: number) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="bg-white p-8 rounded-2xl border border-border-subtle hover:shadow-md transition-all"
                        style={getItemContainerStyle(block)}
                      >
                        {typeof item.icon === "object" ||
                        (typeof item.icon === "string" &&
        ...
        case "quote_image":
          return (
            <section
              key={index}
              className="py-section-gap bg-surface-container-highest/30"
              style={getStyle(block, "")}
            >
              <div className="max-w-container-max mx-auto px-margin-desktop">
                <div className="bg-surface-card rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-xl">
                  <div className="md:w-1/2 h-64 md:h-auto min-h-[350px] relative overflow-hidden">
                    <div
                      className="absolute inset-0 w-full h-full bg-cover bg-center"
                      style={getImageStyle(block, "image")}
                    ></div>
            ...
            >
              ...
              <div className="relative z-10 h-full flex flex-col justify-center px-4 sm:px-10 lg:px-20">
                <div className="max-w-3xl">
                  {block.titlePart1 || block.titlePart2 ? (
                    <h1
                      className="text-white text-4xl md:text-5xl font-black mb-4 tracking-tight"
                      style={getTitleStyle(block)}
                    >
                      {block.titlePart1 && (
        ...
        case "akademik_kadro":
          return (
            <div
              key={index}
              className="max-w-[1440px] mx-auto flex flex-col pt-8 pb-20 px-4 sm:px-10 lg:px-20 gap-8"
              style={getStyle(block, "container")}
            >
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <aside className="w-full lg:w-72 shrink-0">
                  <div
                    className="border border-[#e5e7eb] dark:border-[#2d333d] rounded-xl p-5 sticky top-24"
                    style={{
                      backgroundColor:
                ...
                <div className="flex-1">
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                    {(block.items || []).map((item: any, i: number) => (
                      <div
                        key={i}
                        className="border border-[#e5e7eb] dark:border-[#2d333d] rounded-xl overflow-hidden hover:shadow-lg transition-shadow group"
                        style={{
                          ...getStyle(block, "card"),
                          backgroundColor:
                            item.cardBgColor ||
                        ...
                        >
                          ...
                          {item.tag && (
                            <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/60 to-transparent">
                              <span
                                className="text-white text-xs font-bold px-2 py-1 bg-primary rounded"
                                style={
                                  item.tagColor
        ...
        case "management_hero":
          return (
            <section
              key={index}
              className="relative w-full h-[400px] flex items-center justify-center overflow-hidden mb-12"
              style={getStyle(block, "container")}
            >
              <div className="absolute inset-0 overflow-hidden">
                <div
        ...
        case "management_rector":
          return (
            <div
              key={index}
              className="layout-content-container flex flex-col w-full max-w-[1200px] px-6 py-8 mx-auto"
              style={getStyle(block, "container")}
            >
              <div className="mb-12">
                <div className="flex items-center gap-3 mb-6 border-b border-[#bd0f2c]/20 pb-2">
                ...
                <div className="group relative flex flex-col md:flex-row items-stretch gap-0 rounded-xl bg-white dark:bg-background-dark shadow-xl overflow-hidden border border-slate-100 dark:border-slate-800">
                  <div className="md:w-2/5 relative h-[350px] md:h-auto overflow-hidden">
                    <div className="absolute inset-0 bg-[#bd0f2c]/10 mix-blend-multiply z-10 group-hover:bg-transparent transition-all duration-500"></div>
                    <div className="w-full h-full relative overflow-hidden">
                      <div
                  ...
                  <div className="md:w-3/5 flex flex-col justify-center p-8 md:p-12 gap-6 bg-gradient-to-br from-white to-slate-50 dark:from-background-dark dark:to-slate-900">
                    <div className="space-y-2">
                      <span className="inline-block px-3 py-1 rounded bg-[#bd0f2c] text-white text-xs font-bold uppercase tracking-widest">
                        {block.badge || "Rektörlük Makamı"}
                      </span>
                      <h3 className="text-3xl font-bold text-slate-900 dark:text-white">
                        {block.name || "Prof. Dr. Ahmet Yılmaz"}
                      </h3>
                      <p className="text-[#bd0f2c] font-bold text-lg">
                        {block.role || "Rektör"}
                      </p>
                    </div>
                    <div className="relative">
                      ...
                      <p
                        className="text-slate-600 dark:text-slate-400 text-lg leading-relaxed relative z-10"
        ...
        case "management_vice_rectors":
          return (
            ...
            >
              <div className="mb-12">
                <div className="flex items-center gap-3 mb-8 border-b border-[#bd0f2c]/20 pb-2">
                  {typeof block.icon === "object" ||
                  (typeof block.icon === "string" &&
                    block.icon !== block.icon.toLowerCase()) ? (
                    <IconPreview
                  ...
                  {(block.items || []).map((item: any, i: number) => (
                    ...
                    >
                      ...
                      <div className="p-6 flex flex-col gap-3">
                        <div>
                          <p className="text-xs font-bold text-[#bd0f2c] uppercase tracking-wider mb-1">
                            {item.badge}
                          </p>
                          <h4
                        ...
                        {item.url && (
                          <a
                            className="flex items-center gap-1 text-sm font-bold text-slate-800 dark:text-slate-200 hover:text-[#bd0f2c] transition-colors mt-2"
                            href={item.url}
                          >
        ...
        case "management_deans":
          return (
            ...
            >
              <div className="mb-16">
                <div className="flex items-center justify-between mb-8 border-b border-[#bd0f2c]/20 pb-2">
                  ...
                  {(block.items || []).map((item: any, i: number) => (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <p className="text-[11px] font-bold text-[#bd0f2c] uppercase leading-tight">
                          {item.badge}
                        </p>
                        <h5
                          className="text-md font-bold text-slate-900 dark:text-white leading-snug"
                          style={getCardTitleStyle(item, block)}
                        >
                          {item.name}
                        </h5>
                      ...
                      {item.url && (
                        <div className="mt-4 pt-3 border-t border-slate-50 dark:border-slate-800">
                          <a
                            className="text-xs font-bold text-slate-600 dark:text-slate-400 hover:text-[#bd0f2c] flex items-center gap-1"
                            href={item.url}
                          >
        ...
        case "kindergarten_hero":
          return (
            <section
              key={index}
              className="relative pt-32 pb-section-gap px-margin-desktop bg-surface-bright overflow-hidden"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 items-center relative z-10">
                <div className="space-y-6">
                  {block.badge && (
                ...
                <div className="relative h-[500px] rounded-xl overflow-hidden shadow-sm border border-border-subtle group">
                  <div
              ...
              <div className="absolute top-0 right-0 w-1/3 h-full bg-surface-container-low rounded-l-full -z-10 transform translate-x-1/2"></div>
            </section>
          );
        ...
        case "kindergarten_bento":
          return (
            <section
              key={index}
              className="py-section-gap px-margin-desktop bg-surface-background"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto">
                <div className="text-center mb-16 max-w-2xl mx-auto">
                  <h2
                    className="font-headline-xl text-headline-xl text-on-background mb-4"
                    style={getTitleStyle(block)}
                  >
                  ...
                  {(block.items || []).map((item: any, i: number) => {
                        ...
                        >
                          <div className="relative z-10 flex flex-col h-full justify-between">
                            <div>
                              ...
                              <p
                                className="font-body-md text-body-md text-primary-fixed-dim max-w-md"
                                style={getCardDescStyle(item, block)}
                              >
                                {item.desc}
                              </p>
                          ...
                          <div className="absolute right-0 top-0 w-full h-full bg-gradient-to-l from-primary-fixed-variant/20 to-transparent -z-0"></div>
                        </div>
                      );
                    }
                    ...
                    if (item.rowSpan) {
                      return (
                        <div
                          key={i}
                          className="md:col-span-2 bg-surface-card rounded-xl border border-border-subtle shadow-sm p-8 flex flex-col justify-between group hover:shadow-md transition-shadow relative overflow-hidden"
                          style={getCardStyle(item)}
                        >
                          <div className="relative z-10">
                            {typeof item.icon === "object" ||
                          ...
                          <div className="absolute right-0 bottom-0 w-64 h-64 bg-surface-container-low rounded-tl-full opacity-50 group-hover:scale-110 transition-transform -z-0"></div>
                        </div>
                      );
                    }
                    ...
                    return (
                      <div
                        key={i}
                        className="bg-surface-card rounded-xl border border-border-subtle shadow-sm p-8 flex flex-col justify-between hover:bg-surface-container-low transition-colors group"
                        style={getCardStyle(item)}
                      >
                        <div>
                          {typeof item.icon === "object" ||
        ...
        case "kindergarten_branches":
          return (
            <section
              key={index}
              className="py-section-gap px-margin-desktop bg-surface-bright border-t border-border-subtle"
              style={getStyle(block, "container")}
            >
              <div className="max-w-container-max mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {(block.items || []).map((item: any, i: number) => (
                    <div
                      key={i}
                      className="bg-surface-card rounded-xl border border-border-subtle overflow-hidden shadow-sm hover:shadow-md transition-shadow group"
                      style={getCardStyle(item)}
                    >
                      <div className="h-48 relative overflow-hidden">
                        <div
                          className="absolute inset-0 w-full h-full bg-center bg-cover group-hover:scale-105 transition-transform duration-500"
                          style={getImageStyle(item, "image", i)}
                        ></div>
        ...
        case "primary_school_hero":
          return (
            <header
              key={index}
              className="relative w-full min-h-[614px] flex items-center justify-center bg-surface-container-low overflow-hidden"
              style={getStyle(block, "container")}
            >
              <div className="absolute inset-0 z-0 opacity-40">
                <div
                  ...
                  style={getImageStyle(block, "image")}
                ></div>
                <div className="absolute inset-0 bg-gradient-to-r from-surface-background via-surface-background/80 to-transparent"></div>
              </div>
              <div className="relative z-10 max-w-container-max mx-auto px-margin-desktop w-full grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                <div className="col-span-1 md:col-span-7 lg:col-span-6 space-y-6">
                  {block.badge && (
                  ...
                  {(block.items || []).map((item: any, i: number) => {
                    ...
                    if (item.rowSpan && item.image) {
                      return (
                        <div
                          key={i}
                          className="col-span-1 md:col-span-2 row-span-1 bg-surface-card rounded-xl border border-border-subtle p-8 flex flex-col md:flex-row gap-8 items-center justify-between overflow-hidden relative shadow-sm"
                          style={getCardStyle(item)}
                        >
                          ...
                          <div className="w-full md:w-1/3 h-40 rounded-lg overflow-hidden shrink-0">
                            <div
                              className="w-full h-full bg-cover bg-center"
                              style={getImageStyle(item, "image", i)}
                            ></div>
                    ...
                    if (item.rowSpan) {
                      return (
                        <div
                          key={i}
                          className="col-span-1 md:col-span-2 row-span-1 bg-surface-card rounded-xl border border-border-subtle p-8 flex flex-col justify-between relative overflow-hidden group hover:border-primary/50 transition-colors shadow-sm"
                          style={getCardStyle(item)}
                        >
                          <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full -translate-y-1/2 translate-x-1/4 blur-3xl"></div>
                          <div className="relative z-10">
      ...
      if (block.type === "middle_school_hero") {
        return (
          <section
            key={index}
            className="relative w-full bg-surface-container-low overflow-hidden py-24 md:py-32"
            style={getStyle(block, "container")}
          >
            <div className="absolute inset-0 z-0">
            ...
            <div className="max-w-container-max mx-auto px-margin-desktop relative z-10 flex flex-col items-start w-full md:w-2/3">
              {block.badge && (
                <span className="inline-block py-1 px-3 rounded-full bg-primary/10 text-primary font-label-md text-label-md mb-6 border border-primary/20">
                  {block.badge}
                </span>
              )}
              <h1
                className="font-display-lg text-display-lg text-on-surface mb-6 leading-tight"
                style={getTitleStyle(block)}
              >
                {block.titlePart1 || block.title}{" "}
                {block.titlePart2 && (
              ...
              {block.subtitle && (
                <p
                  className="font-body-lg text-body-lg text-on-surface-variant mb-10 max-w-2xl"
                  style={getSubtitleStyle(block)}
                >
                  {block.subtitle}
                </p>
            ...
            <div className="max-w-container-max mx-auto px-margin-desktop">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                <div className="md:col-span-5 order-2 md:order-1 relative">
                  <div className="absolute -inset-4 bg-secondary/5 rounded-2xl -z-10"></div>
                  <div
                    className="w-full aspect-[4/5] bg-cover bg-center rounded-xl border border-border-subtle shadow-sm"
                    style={getImageStyle(block, "image")}
                  ></div>
                </div>
                <div className="md:col-span-7 order-1 md:order-2 md:pl-12">
                  <h2
                    className="font-headline-xl text-headline-xl text-on-surface mb-6"
                    style={getTitleStyle(block)}
                  >
                    {block.title}
                  </h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                    {(block.items || []).map((item: any, i: number) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="bg-surface-card p-6 rounded-xl border border-border-subtle shadow-sm hover:shadow-md transition-shadow"
                          style={getCardStyle(item)}
                        >
                          <div
                          ...
                          <h3
                            className="font-headline-md text-headline-md text-on-surface mb-2 text-[20px]"
                            style={getCardTitleStyle(item, block)}
                          >
                            {item.title}
                          </h3>
                ...
                {(block.items || []).map((item: any, i: number) => {
                  if (item.rowSpan && item.image) {
                    return (
                      <div
                        key={i}
                        className="md:col-span-2 md:row-span-2 bg-surface-card rounded-xl border border-border-subtle overflow-hidden relative group"
                        style={getCardStyle(item)}
                      >
                        <div
                        ...
                        <div className="absolute bottom-0 left-0 p-8 w-full">
                          {item.badge && (
                            <span className="inline-block py-1 px-3 rounded-md bg-primary text-on-primary font-label-sm text-label-sm mb-3">
                              {item.badge}
                            </span>
                          )}
                          <h3
                          ...
                          <p
                            className="font-body-md text-body-md text-white/80 max-w-xl"
                            style={getCardDescStyle(item, block)}
                          >
                            {item.desc}
                          </p>
                  ...
                  return (
                    <div
                      key={i}
                      className="bg-surface-card p-6 rounded-xl border border-border-subtle shadow-sm flex flex-col justify-between"
                      style={getCardStyle(item)}
                    >
                      <div
                      ...
                      <div>
                        <h3
                          className="font-headline-md text-headline-md text-on-surface mb-2 text-[18px]"
                          style={getCardTitleStyle(item, block)}
                        >
                          {item.title}
                        </h3>
      ...
      if (block.type === "high_school_hero") {
        return (
          <section
            key={index}
            className="relative w-full py-section-gap overflow-hidden bg-surface-card border-b border-border-subtle"
            style={getStyle(block, "container")}
          >
            <div className="max-w-container-max mx-auto px-margin-desktop grid md:grid-cols-2 gap-12 items-center">
              <div className="z-10 flex flex-col gap-6">
                ...
                <h1
                  className="font-display-lg text-display-lg text-main"
                  style={getTitleStyle(block)}
                >
                  {block.titlePart1 || block.title}{" "}
                  {block.titlePart2 && (
              ...
              <div className="relative h-[500px] w-full rounded-2xl overflow-hidden border border-border-subtle shadow-sm group">
                <div
                  className="absolute inset-0 w-full h-full bg-cover bg-center transition-transform duration-700 group-hover:scale-105"
                  style={getImageStyle(block, "image")}
                ></div>
        ...
        return (
          ...
          >
            <div className="max-w-container-max mx-auto px-margin-desktop">
              <div className="text-center mb-16">
                <h2
                  className="font-headline-xl text-headline-xl text-main mb-4"
                  style={getTitleStyle(block)}
                >
                ...
                {block.subtitle && (
                  <p
                    className="font-body-md text-body-md text-text-muted max-w-2xl mx-auto"
                    style={getSubtitleStyle(block)}
                  >
                    {block.subtitle}
                  </p>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {(block.items || []).map((item: any, i: number) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="bg-surface-card border border-border-subtle rounded-xl p-8 hover:shadow-sm transition-all group cursor-pointer relative overflow-hidden"
                      style={getCardStyle(item)}
                    >
                      <div
                      ...
                      <div className="flex items-center gap-4 mb-6">
                        ...
                        <h3
                          className="font-headline-md text-headline-md text-main"
                          style={getCardTitleStyle(item, block)}
                        >
                          {item.title}
                        </h3>
        ...
        return (
          ...
          >
            <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 gap-6 relative z-10 py-20 items-center w-full">
              <div className="space-y-6 text-center mx-auto">
                {block.badge && (
                  <span className="inline-block bg-[#3f68e4]/20 text-[#b6c4ff] px-4 py-1 rounded-full text-[12px] font-bold tracking-[0.05em]">
                    {block.badge}
                  </span>
                )}
                ...
                {block.subtitle && (
                  <p className="text-lg md:text-[18px] text-[#dce1ff] max-w-xl mx-auto">
                    {block.subtitle}
                  </p>
                )}
                  ...
                  <div className="flex flex-wrap gap-4 pt-4 justify-center">
                    {block.buttons.map((btn: any, i: number) => (
                      <a
                        ...
                        href={btn.url || "#"}
                        style={getIndividualButtonStyle(btn)}
                        className="px-8 py-3 rounded-xl text-[14px] font-bold flex items-center gap-2 hover:opacity-90 transition-opacity"
                      >
                        {btn.label}
                        {btn.icon &&
                          (typeof btn.icon === "string" &&
      ...
      if (block.type === "achievements_academic_bento") {
        return (
          <section
            key={block.id}
            className="py-20 bg-[#faf8ff]"
            style={getStyle(block, "")}
          >
            <div className="max-w-7xl mx-auto px-8">
              <div className="text-center mb-16">
                <h2 className="font-bold text-[36px] text-[#1a1b23] mb-4">
                  {block.title}
                </h2>
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.buttonText && (
                          <a
                            href={item.url || "#"}
                            className="mt-8 block w-full text-center py-3 bg-[#D4AF37] text-[#00164f] font-bold rounded-xl hover:bg-[#D4AF37]/90 transition-colors"
                          >
                            {item.buttonText}
                          </a>
                        )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="space-y-6">
                          {item.statValue && (
                            <div className="flex flex-col gap-1">
                              <div className="flex items-end gap-3">
                                ...
                                {item.statLabel && (
                                  <span className="text-[14px] font-semibold text-[#434654] pb-1">
                                    {item.statLabel}
                                  </span>
                                )}
                              </div>
                          ...
                          {item.badge && (
                            <p className="text-[12px] font-bold tracking-[0.05em] text-[#1d4eca]">
                              {item.badge}
                            </p>
                          )}
                        </div>
                ...
                {(block.items || []).map((item: any, idx: number) => (
                  <div key={idx} className="group cursor-pointer">
                    <div className="aspect-[3/4] rounded-2xl overflow-hidden mb-4 relative">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
                        <p className="text-white text-[12px] font-bold tracking-[0.05em]">
                          {item.hoverText}
                        </p>
                      </div>
                    </div>
                    ...
                    <p className="text-[#434654] text-[12px] font-bold tracking-[0.05em]">
                      {item.desc}
                    </p>
                  </div>
                ))}
      ...
      if (block.type === "achievements_science_projects") {
        return (
          <section
            key={block.id}
            className="py-20 bg-[#f3f2fd] overflow-hidden"
            style={getStyle(block, "")}
          >
            <div className="max-w-7xl mx-auto px-8">
              <div className="bg-[#0f172a] rounded-[40px] p-8 md:p-20 relative overflow-hidden text-white">
                ...
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                  <div>
                    {block.badge && (
                      <span className="inline-block bg-[#D4AF37]/20 text-[#D4AF37] px-4 py-1 rounded-full text-[12px] font-bold tracking-[0.05em] mb-6">
                        {block.badge}
                      </span>
                    )}
                    <h2 className="font-bold text-3xl md:text-[36px] mb-6 leading-tight">
                      {block.title}
                    </h2>
                    {block.subtitle && (
                      <p className="text-[#dce1ff] mb-10 text-lg">
                        {block.subtitle}
                      </p>
                    )}
                  ...
                  <div className="relative mt-10 md:mt-0">
                    {block.image && (
                      <img
                        className="rounded-3xl shadow-2xl border border-white/10 w-full object-cover aspect-video"
                        src={block.image}
                        alt={block.title}
                      />
                    )}
          ...
          <section key={index} className="relative h-[400px] flex items-center overflow-hidden" style={getStyle(block, 'container')}>
            ...
            <div className="max-w-container-max mx-auto px-margin-desktop relative z-10 w-full">
              <div className="max-w-2xl">
                ...
                <h1 className="text-4xl lg:text-6xl font-black text-white mb-6 leading-tight" style={getTitleStyle(block)}>{block.title}</h1>
                {block.subtitle && (
        ...
        return (
          <section key={index} className="py-12 bg-surface-background" style={getStyle(block, 'container')}>
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10 max-w-container-max mx-auto px-margin-desktop">
              <div>
                <h2 className="text-2xl font-bold text-slate-800" style={getTitleStyle(block)}>{block.title}</h2>
