...
import { DEFAULT_PRE_REGISTRATION_INPUTS, DEFAULT_CLUB_INPUTS, DEFAULT_SCHOLARSHIP_INPUTS, DEFAULT_CAREER_INPUTS, DEFAULT_CONTACT_INPUTS } from "../lib/defaultFormInputs";
  ...
  return (
    <div key={block.id || "clubs-grid"} style={getStyle(block, "container")}>
      <section
        ...
        id="clubs"
      >
        <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-gutter`}>
      ...
      <section className={`py-section-gap ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-gutter`}>
              ...
              >
                ...
                <div className="p-6 flex-grow flex flex-col whitespace-pre-line">
                  <div className="flex items-center gap-2 mb-2 whitespace-pre-line">
                    {item.icon &&
                      ...
                      item.icon === item.icon.toLowerCase() ? (
                        <IconPreview data={item.icon} className="text-primary text-[24px] whitespace-pre-line" style={{ fontVariationSettings: "'FILL' 1"  }} />
                      ) : (
                        <IconPreview
                          data={item.icon}
                  ...
                  <div className="mt-auto flex items-center justify-between whitespace-pre-line">
                    ...
                    >
                      Detaylı Bilgi
                      <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">
                        arrow_forward
                      </span>
                    </a>
                  </div>
...
const CareerHeroBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  return (
    <section key={index} className="relative w-full rounded-2xl overflow-hidden min-h-[400px] flex items-center justify-center mb-8 whitespace-pre-line" style={getStyle(block, "container")}>
      <div 
      ...
      <div className={`relative z-10 ${block.styles?.textAlign ? "" : "text-center"} px-6 py-16 md:py-24 text-white max-w-3xl mx-auto`}>
        <h1 className="font-display-lg text-display-lg font-black text-white mb-6 leading-[1.2] tracking-tight whitespace-pre-line" style={getTitleStyle(block)}>
          {block.title || "Dost Koleji'nde Kariyer"}
        </h1>
        <p className="font-body-lg text-[18px] text-white/90 mb-8 max-w-2xl mx-auto whitespace-pre-line" style={getSubtitleStyle(block)}>
        ...
        >
          ...
          <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">arrow_downward</span>
        </a>
      </div>
    </section>
  );
...
const CareerBenefitsBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="w-full px-4 md:px-0 mb-16 whitespace-pre-line" style={getStyle(block, "container")}>
      <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-12`}>
        <h2 className="font-bold text-[28px] md:text-[36px] text-[#002147] mb-4 whitespace-pre-line" style={getTitleStyle(block)}>
          {block.title || "Neden Bize Katılmalısınız?"}
        </h2>
        <p className="font-normal text-[16px] text-slate-500 max-w-2xl mx-auto whitespace-pre-line" style={getSubtitleStyle(block)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 whitespace-pre-line">
        {items.map((item: any, i: number) => (
          <div key={i} className={`bg-white border border-slate-200 rounded-xl p-6 transition-transform hover:-translate-y-1 hover:shadow-md ${item.borderTop || ''}`}>
            ...
            <h3 className="font-bold text-[20px] text-slate-900 mb-2 whitespace-pre-line">{item.title}</h3>
            <p className="font-normal text-[16px] text-slate-500 whitespace-pre-line">{item.desc}</p>
          </div>
        ))}
      </div>
    </section>
...
const CareerApplicationBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} id="application-form" className="w-full px-4 md:px-0 mb-16 whitespace-pre-line" style={getStyle(block, "container")}>
      <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-7xl"} mx-auto flex flex-col lg:flex-row gap-12`}>
        
        {/* Left Side: Open Positions */}
        <div className="lg:w-1/3 whitespace-pre-line">
          <div className="sticky top-24 whitespace-pre-line">
            <h2 className="font-bold text-[28px] text-[#002147] mb-6 whitespace-pre-line" style={getTitleStyle(block)}>
              {block.title || "Açık Pozisyonlar"}
            </h2>
            {positions.length > 0 ? (
              <div className="space-y-4 whitespace-pre-line">
                {positions.map((pos: any, idx: number) => (
                  <div key={idx} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-[#D4AF37] hover:shadow-md transition-all group whitespace-pre-line">
                    <div className="flex items-start justify-between mb-2 whitespace-pre-line">
                      <h4 className="font-bold text-[#002147] group-hover:text-[#D4AF37] transition-colors whitespace-pre-line">{pos.title}</h4>
                      <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded whitespace-pre-line">{pos.type}</span>
                    </div>
                    <p className="text-[13px] text-slate-500 whitespace-pre-line">{pos.dept}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className={`bg-slate-50 text-slate-500 p-6 rounded-xl ${block.styles?.textAlign ? "" : "text-center"} text-sm border border-slate-100`}>
        ...
        <div className="lg:w-2/3 whitespace-pre-line">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 relative whitespace-pre-line">
             {/* Header */}
            <div className={`bg-[#002147] px-8 py-6 text-white ${block.styles?.textAlign ? "" : "text-center"}`}>
              <h2 className="font-bold text-[24px] whitespace-pre-line">İş Başvurusu</h2>
              <p className="text-white/80 mt-2 whitespace-pre-line">Dost Koleji ailesine katılmak için formu doldurun</p>
            </div>
...
const EduSystemHeroBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="relative w-full min-h-[500px] flex items-center justify-center overflow-hidden whitespace-pre-line" style={getStyle(block, "container")}>
      ...
      <div className={`relative z-20 w-full ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} px-margin-mobile md:px-margin-desktop py-20 ${block.styles?.textAlign ? "" : "text-center"} text-white`}>
        <h1 className="font-display-lg text-display-lg font-extrabold mb-6 whitespace-pre-line" style={getTitleStyle(block)}>
          {block.title || 'Eğitim Sistemimiz'}
        </h1>
        <p className="font-body-lg text-lg md:text-xl max-w-3xl mx-auto text-surface-bright/90 whitespace-pre-line" style={getSubtitleStyle(block)}>
...
const EduSystemLevelsBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  return (
    <section key={index} className={`py-section-gap px-margin-mobile md:px-margin-desktop ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto`} style={getStyle(block, "container")}>
      <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-16`}>
        <h2 className="font-headline-xl text-2xl md:text-4xl font-bold text-text-main mb-4 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 whitespace-pre-line">
        {(block.items || []).map((item: any, i: number) => {
          ...
          return (
            <div key={i} className="bg-surface-card rounded-xl border border-border-subtle p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full whitespace-pre-line"
              style={{
            ...
            >
              <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-6 transition-colors ${isPrimary ? 'bg-primary/10 group-hover:bg-primary text-primary' : 'bg-secondary/10 group-hover:bg-secondary text-secondary'} group-hover:text-white`}>
                <IconPreview data={item.icon || 'school'} className="text-3xl transition-colors whitespace-pre-line" />
              </div>
              <h3 className="font-headline-md text-xl font-bold text-text-main mb-3 whitespace-pre-line" style={{color: item.itemTitleColor}}>{item.title}</h3>
              <p className="font-body-md text-text-muted mb-6 flex-grow whitespace-pre-line" style={{color: item.itemDescColor}}>{item.desc}</p>
              {item.url && !item.hideButton && (
                <SmartLink className={`inline-flex items-center font-label-md font-semibold hover:opacity-80 group/link ${isPrimary ? 'text-primary' : 'text-secondary'}`} url={item.url}>
                  ...
                  <span className="material-symbols-outlined text-sm ml-1 group-hover/link:translate-x-1 transition-transform whitespace-pre-line" translate="no" aria-hidden="true">arrow_forward</span>
                </SmartLink>
              )}
            </div>
          );
...
const EduSystemYadepBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  return (
    <section key={index} className="py-section-gap bg-surface-container-low whitespace-pre-line" style={getStyle(block, "container")}>
      <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-mobile md:px-margin-desktop`}>
        <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-16`}>
          <h2 className="font-headline-xl text-2xl md:text-4xl font-bold text-text-main mb-4 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
          <p className="font-body-lg text-text-muted max-w-2xl mx-auto whitespace-pre-line" style={getSubtitleStyle(block)}>{block.subtitle}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 whitespace-pre-line">
          {(block.items || []).map((item: any, i: number) => {
             ...
             return (
              <div key={i} className="bg-surface-card p-8 rounded-xl shadow-sm border border-border-subtle hover:shadow-md transition-shadow whitespace-pre-line"
                style={{
                  backgroundColor: item.cardBgColor || undefined,
                  borderColor: item.cardBorderColor || undefined,
                  borderWidth: item.cardBorderWidth || undefined,
              ...
              >
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 ${
                  ...
                  <IconPreview data={item.icon} className="text-3xl whitespace-pre-line" />
                </div>
                <h3 className="font-headline-md text-xl font-bold text-text-main mb-3 whitespace-pre-line" style={{color: item.itemTitleColor}}>{item.title}</h3>
                <p className="font-body-md text-text-muted whitespace-pre-line" style={{color: item.itemDescColor}}>{item.desc}</p>
              </div>
            );
          })}
        </div>
...
const EduSystemPhilosophyBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="py-section-gap bg-surface-container relative overflow-hidden whitespace-pre-line" style={getStyle(block, "container")}>
      ...
      <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-mobile md:px-margin-desktop relative z-10`}>
        <div className="flex flex-col lg:flex-row gap-16 items-center whitespace-pre-line">
          <div className="lg:w-1/2 space-y-8 whitespace-pre-line">
            <div>
              <span className="text-secondary font-label-md tracking-wider uppercase mb-2 block whitespace-pre-line">{block.badge}</span>
              <h2 className="font-headline-xl text-2xl md:text-4xl font-bold text-text-main whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
            </div>
            <p className="font-body-md text-text-muted text-lg whitespace-pre-line" style={getSubtitleStyle(block)}>
              ...
              {(block.items || []).map((item: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex gap-4 whitespace-pre-line">
                    ...
                    <div>
                      <h4 className="font-headline-md text-lg font-bold text-text-main mb-1 whitespace-pre-line" style={{color: item.itemTitleColor}}>{item.title}</h4>
                      <p className="font-body-md text-text-muted text-sm whitespace-pre-line" style={{color: item.itemDescColor}}>{item.desc}</p>
                    </div>
                  </div>
                );
              })}
            ...
            {block.cardTitle && (
              <div className="absolute -bottom-8 -left-8 md:-bottom-12 md:-left-12 bg-white/80 backdrop-blur-md p-6 rounded-xl border border-white shadow-lg max-w-xs hidden sm:block whitespace-pre-line">
                <div className="flex items-center gap-3 mb-2 whitespace-pre-line">
                  <span className="material-symbols-outlined text-secondary text-3xl whitespace-pre-line" translate="no" aria-hidden="true">{block.cardIcon || 'emoji_events'}</span>
                  <span className="font-headline-md font-bold text-text-main whitespace-pre-line">{block.cardTitle}</span>
                </div>
                <p className="font-body-md text-sm text-text-muted whitespace-pre-line">{block.cardDesc}</p>
              </div>
            )}
          </div>
        </div>
...
const EduSystemCtaBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  return (
    <section key={index} className="py-24 bg-primary text-white relative overflow-hidden whitespace-pre-line" style={getStyle(block, "container")}>
      <div className="absolute inset-0 opacity-10 pointer-events-none whitespace-pre-line" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, white 1px, transparent 0)', backgroundSize: '32px 32px' }}></div>
      <div className={`max-w-4xl mx-auto px-margin-mobile ${block.styles?.textAlign ? "" : "text-center"} relative z-10`}>
        <h2 className="font-headline-xl text-2xl md:text-4xl font-bold mb-6 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
        <p className="font-body-lg text-lg text-primary-fixed-dim mb-10 max-w-2xl mx-auto whitespace-pre-line" style={getSubtitleStyle(block)}>
          {block.subtitle || block.desc}
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 whitespace-pre-line">
          {(block.buttons || []).map((btn: any, btnIdx: number) => (
            <SmartLink key={btnIdx} url={btn.url} className="w-full sm:w-auto px-8 py-4 bg-white text-primary font-label-md font-bold rounded-lg hover:bg-surface-bright hover:shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 whitespace-pre-line">
...
const DynamicFormBuilder = ({ block, type, submitForm }: any) => {
  const defaultInputs = block.inputs && block.inputs.length > 0 ? block.inputs : (type === 'club_registration_form' ? DEFAULT_CLUB_INPUTS : type === 'bursluluk_exam_form' ? DEFAULT_SCHOLARSHIP_INPUTS : type === 'career_application' ? DEFAULT_CAREER_INPUTS : type === 'contact_form' ? DEFAULT_CONTACT_INPUTS : DEFAULT_PRE_REGISTRATION_INPUTS);
    ...
    <div className={type === "club_registration_form" ? "bg-surface-card border border-border-subtle rounded-xl p-6 md:p-10 shadow-sm relative form-card" : "relative"} style={type === "club_registration_form" && block.styles?.cardBgColor ? { backgroundColor: block.styles.cardBgColor } : {}}>
      {submitted ? (
        type === "bursluluk_exam_form" ? (
          <div className={`p-8 md:p-12 ${block.styles?.textAlign ? "" : "text-center"} min-h-[400px] flex flex-col items-center justify-center space-y-6`}>
            <div className="w-20 h-20 bg-primary/10 text-primary rounded-full flex items-center justify-center mx-auto shadow-inner whitespace-pre-line">
              <span className="material-symbols-outlined text-4xl whitespace-pre-line" translate="no" aria-hidden="true">check_circle</span>
            </div>
            <h3 className="text-2xl font-bold text-[#002147] whitespace-pre-line">Bursluluk Sınavı Başvurunuz Başarıyla Alındı!</h3>
            ...
            >
              <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">badge</span>
              Sınav Giriş Belgesini Görüntüle ve İndir
            </a>
          </div>
        ) : type === "contact_form" ? (
          <div className={`p-12 ${block.styles?.textAlign ? "" : "text-center"} bg-green-50 rounded-xl border border-green-100 min-h-[300px] flex flex-col items-center justify-center`}>
            <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4 whitespace-pre-line">
              <span className="material-symbols-outlined text-3xl whitespace-pre-line" translate="no" aria-hidden="true">check_circle</span>
            </div>
            <h3 className="text-xl font-bold text-green-800 mb-2 whitespace-pre-line">Mesajınız İletildi</h3>
            <p className="text-green-700 whitespace-pre-line">Mesajınız başarıyla gönderilmiştir. En kısa sürede sizinle iletişime geçeceğiz.</p>
          </div>
        ) : (
          <div className={`p-12 ${block.styles?.textAlign ? "" : "text-center"} min-h-[400px] flex flex-col items-center justify-center`}>
            ...
            <h3 className="font-headline-md text-headline-md text-on-surface mb-2 whitespace-pre-line">Başvurunuz Alındı</h3>
            <p className="font-body-md text-body-md text-text-muted whitespace-pre-line">Kayıt başvurunuz başarıyla alınmıştır.</p>
          </div>
        )
      ) : (
        <form onSubmit={handleSubmit} className={isStyledForm ? "p-6 md:p-10 space-y-10 text-left" : "space-y-8"}>
          ...
          {defaultInputs.length === 0 ? (
            <div className={`p-6 ${block.styles?.textAlign ? "" : "text-center"} text-text-muted border border-dashed border-border-subtle rounded-lg`}>
                  ...
                  if (isStyledForm) {
                    return (
                      <div key={inputKey} className={`flex items-center gap-3 mb-6 border-b border-border-subtle pb-2 mt-4 first:mt-0 ${colSpan}`}>
                        {input.icon && <IconPreview data={input.icon} className="text-primary whitespace-pre-line" />}
                        <h2 className="font-label-md text-label-md text-text-main uppercase tracking-wider whitespace-pre-line">{input.label}</h2>
                      </div>
                    );
                  }
                  return (
                    <div key={inputKey} className={`flex items-center gap-2 border-b border-border-subtle pb-3 mt-4 first:mt-0 ${colSpan}`}>
                      {input.icon && <IconPreview data={input.icon} className="text-primary whitespace-pre-line" />}
                      <h2 className="font-headline-md text-headline-md text-on-surface whitespace-pre-line">{input.label}</h2>
                    </div>
                  );
                }
                    ...
                    <div key={inputKey} className={`pt-2 border-t border-border-subtle ${colSpan}`}>
                      <label className="flex items-start gap-3 cursor-pointer group whitespace-pre-line">
                        ...
                        <span className="font-body-md text-body-md text-on-surface-variant group-hover:text-on-surface transition-colors whitespace-pre-line">
                          <span className="font-bold text-primary whitespace-pre-line">{input.label.split(' ')[0]}</span> {input.label.substring(input.label.indexOf(' ') + 1)}
                        </span>
                      </label>
                    </div>
                  );
                ...
                if (input.type === 'radio') {
                  ...
                  return (
                    <div key={inputKey} className={`space-y-2 ${colSpan}`}>
                      <label className="font-label-md text-label-md text-on-surface-variant block whitespace-pre-line">{input.label}</label>
                      <div className="space-y-2 whitespace-pre-line">
                        {opts.map((opt: string, optIdx: number) => (
                          <label key={`${inputKey}_opt_${optIdx}`} className="flex items-center gap-2 cursor-pointer whitespace-pre-line">
                            <input 
                            ...
                            />
                            <span className="font-body-md text-body-md text-on-surface whitespace-pre-line">{opt}</span>
                          </label>
                        ))}
                      </div>
                    </div>
              ...
              {type === 'club_registration_form' && defaultClubs && defaultClubs.length > 0 && (
                <div className="md:col-span-2 space-y-6 mt-0 whitespace-pre-line">
                  <div className="flex items-center gap-2 border-b border-border-subtle pb-3 whitespace-pre-line">
                    <span className="material-symbols-outlined text-primary whitespace-pre-line" translate="no" aria-hidden="true">explore</span>
                    <h2 className="font-headline-md text-headline-md text-on-surface whitespace-pre-line">Kulüp Seçimi</h2>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 whitespace-pre-line">
                    {defaultClubs.map((clubOpt: any, clubIdx: number) => {
                      ...
                      return (
                        <div key={clubId} className="relative whitespace-pre-line">
                          <input 
          ...
          <div className="pt-6 whitespace-pre-line">
            ...
            >
              ...
              {!submitting && <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">send</span>}
              {submitting && (
                <svg className="animate-spin h-5 w-5 text-white whitespace-pre-line" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25 whitespace-pre-line" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75 whitespace-pre-line" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
              )}
            </button>
            {isStyledForm && (
              <p className={`mt-4 ${block.styles?.textAlign ? "" : "text-center"} font-caption text-caption text-text-muted px-4`}>
...
const ClubRegistrationFormBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  return (
    <section key={index} className="py-section-gap w-full flex items-center justify-center p-4 md:p-8 whitespace-pre-line" style={getStyle(block, "container")}>
      <div className="w-full max-w-[640px] animate-in fade-in slide-in-from-bottom-4 duration-700 whitespace-pre-line">
        <div className={`mb-8 ${block.styles?.textAlign ? "" : "text-center"}`}>
          <h1 className="font-display-lg text-display-lg text-primary mb-2 whitespace-pre-line">{block.title || "Dost Koleji"}</h1>
          <p className="font-body-lg text-body-lg text-text-muted whitespace-pre-line">{block.subtitle || "Öğrenci Kulüp Kayıt Portalı"}</p>
        </div>
...
const PreRegistrationFormBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="py-section-gap w-full flex items-center justify-center p-4 md:p-8 whitespace-pre-line" style={getStyle(block, "container")}>
      <div className="w-full max-w-4xl bg-surface-card rounded-lg shadow-sm border border-border-subtle overflow-hidden relative whitespace-pre-line" style={block.styles?.cardBgColor ? { backgroundColor: block.styles.cardBgColor } : {}}>
        
        {/* Header */}
        <div className={`p-8 md:p-12 ${block.styles?.textAlign ? "" : "text-center"} relative overflow-hidden`} style={{ backgroundColor: block.styles?.headerBgColor || '#002147' }}>
          <div className="relative z-10 whitespace-pre-line">
            <h1 className="font-headline-md text-headline-md text-white mb-2 uppercase tracking-wide whitespace-pre-line" style={getTitleStyle(block)}>
              {block.title || "ÖĞRENCİ ÖN KAYIT FORMU"}
            </h1>
            <p className="font-body-md text-body-md text-blue-200 whitespace-pre-line" style={getSubtitleStyle(block)}>
              {block.subtitle || "Lütfen Formu Eksiksiz Doldurunuz."}
            </p>
            <div className="mt-6 flex justify-center whitespace-pre-line">
              <div className="h-1 w-20 bg-primary rounded-full whitespace-pre-line" style={block.styles?.titlePart1Color ? { backgroundColor: block.styles.titlePart1Color } : {}}></div>
            </div>
          </div>
        </div>
...
const BurslulukHeroBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="relative h-[550px] md:h-[600px] flex items-center overflow-hidden whitespace-pre-line" style={getStyle(block, "container")}>
      <div className="absolute inset-0 z-0 whitespace-pre-line">
      ...
      <div className={`relative z-20 ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop w-full`}>
        <div className={getHeroInnerClass(block, "max-w-2xl text-white")}>
          ...
          <h1 className="font-display-lg text-3xl sm:text-4xl md:text-display-lg mb-6 leading-tight font-extrabold whitespace-pre-line" style={getTitleStyle(block)}>
            {block.title || "Akademik Başarıya Giden Yolunuz"}
          </h1>
          <p className="font-body-lg text-base md:text-body-lg mb-8 opacity-90 leading-relaxed whitespace-pre-line" style={getSubtitleStyle(block)}>
          ...
          <div className="mt-8 flex flex-wrap gap-8 border-l-2 border-[#D4AF37] pl-6 whitespace-pre-line">
            {stats.map((stat: any, idx: number) => (
              <div key={idx}>
                <p className="text-[#D4AF37] font-bold text-xl md:text-2xl whitespace-pre-line">{stat.value}</p>
                <p className="text-sm opacity-80 whitespace-pre-line">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
...
const BurslulukExamFormBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} id="basvuru-formu" className="py-section-gap w-full flex items-center justify-center p-4 md:p-8 whitespace-pre-line" style={getStyle(block, "container")}>
      <div className="w-full max-w-4xl bg-surface-card rounded-xl shadow-sm border border-border-subtle overflow-hidden relative whitespace-pre-line" style={block.styles?.cardBgColor ? { backgroundColor: block.styles.cardBgColor } : {}}>
        ...
        {burslulukActive ? (
          ...
          <div className="p-8 md:p-12 whitespace-pre-line">
            <div className={`${block.styles?.textAlign ? "" : "text-center"} bg-blue-50 border border-blue-100 p-8 rounded-xl`}>
              <span className="material-symbols-outlined text-blue-500 text-5xl mb-4 whitespace-pre-line" translate="no" aria-hidden="true">info</span>
...
const BurslulukInfoCardsBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} id="bilgilendirme" className="py-section-gap bg-white whitespace-pre-line" style={getStyle(block, "container")}>
      <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
        {block.title && (
          <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-12`}>
            <h2 className="font-headline-xl text-2xl md:text-headline-xl text-[#002147] mb-4 font-bold whitespace-pre-line" style={getTitleStyle(block)}>
              {block.title}
            </h2>
            {block.subtitle && (
              <p className="text-slate-500 max-w-2xl mx-auto text-sm md:text-base whitespace-pre-line" style={getSubtitleStyle(block)}>
                {block.subtitle}
              </p>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 whitespace-pre-line">
          {items.map((item: any, idx: number) => (
            <div key={idx} className="group p-8 rounded-2xl bg-[#f3f2fd] border border-[#e2e8f0] hover:border-[#1d4eca]/30 transition-all shadow-sm hover:shadow-md whitespace-pre-line">
              <div className="w-14 h-14 bg-white rounded-xl shadow-sm flex items-center justify-center mb-6 group-hover:scale-110 transition-transform whitespace-pre-line">
                <IconPreview data={item.icon || 'info'} className="text-[#1d4eca] text-3xl whitespace-pre-line" />
              </div>
              <h4 className="text-xl font-bold text-[#002147] mb-4 whitespace-pre-line">{item.title}</h4>
              <ul className="space-y-3 text-[#434654] whitespace-pre-line">
                {(item.rules || item.desc || []).map((rule: string, rIdx: number) => (
                  <li key={rIdx} className="flex items-start gap-2 text-sm leading-relaxed whitespace-pre-line">
                    <span className="material-symbols-outlined text-[#1d4eca] text-base mt-0.5 shrink-0 whitespace-pre-line" translate="no" aria-hidden="true">check_circle</span>
                    <span>{rule}</span>
                  </li>
                ))}
              </ul>
...
const BurslulukResultQueryBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="py-section-gap bg-[#e2e1ec]/20 whitespace-pre-line" style={getStyle(block, "container")}>
      <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12 bg-[#002147] rounded-[40px] overflow-hidden shadow-xl whitespace-pre-line">
          <div className="flex-1 p-8 md:p-12 text-white whitespace-pre-line">
            <h2 className="font-headline-xl text-2xl md:text-headline-xl mb-4 md:mb-6 font-bold whitespace-pre-line" style={getTitleStyle(block)}>
              {block.title || "Sınav Sonuç Sorgulama"}
            </h2>
            <p className="text-base md:text-lg opacity-80 mb-8 whitespace-pre-line" style={getSubtitleStyle(block)}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md whitespace-pre-line">
              ...
              >
                <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">search</span>
      ...
      {queryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 whitespace-pre-line">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative whitespace-pre-line">
            ...
            >
              <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">close</span>
            </button>
            <h3 className="text-2xl font-bold text-[#002147] mb-2 whitespace-pre-line">Sınav Sonuç Sorgulama</h3>
            <p className="text-slate-500 text-sm mb-6 whitespace-pre-line">Lütfen öğrencinin T.C. Kimlik Numarasını giriniz.</p>
            <input 
              type="text" 
              maxLength={11} 
              value={tcQuery} 
...
const BurslulukConfirmationBlock = ({ block, index, getStyle, getTitleStyle, getSubtitleStyle }: any) => {
  ...
  return (
    <section key={index} className="py-12 md:py-16 bg-surface-background min-h-screen whitespace-pre-line" style={getStyle(block, "container")}>
      <style>{`
        @media print {
          body * {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 whitespace-pre-line">
        ...
        {/* Success Header */}
        <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-10 no-print`}>
          <div className="w-20 h-20 bg-primary/10 text-primary rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner whitespace-pre-line">
            <span className="material-symbols-outlined text-5xl whitespace-pre-line" translate="no" aria-hidden="true">{block.successIcon || "check_circle"}</span>
          </div>
          <h1 className="font-headline-xl text-2xl md:text-4xl text-[#002147] mb-3 font-bold whitespace-pre-line" style={getTitleStyle(block)}>
            {block.title || "Başvurunuz Başarıyla Alındı!"}
          </h1>
          <p className="text-slate-600 max-w-2xl mx-auto text-sm md:text-base leading-relaxed whitespace-pre-line" style={getSubtitleStyle(block)}>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8 no-print whitespace-pre-line">
          ...
          >
            <span className="material-symbols-outlined text-xl whitespace-pre-line" translate="no" aria-hidden="true">download</span> İndir (PDF)
          </button>
          <button 
            type="button"
            onClick={handlePrint}
          ...
          >
            <span className="material-symbols-outlined text-xl whitespace-pre-line" translate="no" aria-hidden="true">print</span> Yazdır
          </button>
        </div>
          ...
          <div className="h-full flex flex-col whitespace-pre-line">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b-2 border-[#002147] pb-6 mb-8 gap-4 whitespace-pre-line">
              <div className="flex items-center gap-4 whitespace-pre-line">
                ...
                />
                <div>
                  <h3 className="font-bold text-[#002147] text-lg uppercase tracking-wider whitespace-pre-line">{block.documentTitle || "Bursluluk Sınavı Giriş Belgesi"}</h3>
                  <p className="text-[11px] text-slate-500 uppercase tracking-widest font-semibold whitespace-pre-line">DOST KOLEJİ EĞİTİM KURUMLARI</p>
                </div>
              </div>
              <div className="text-left sm:text-right border-l sm:border-l-0 border-slate-200 pl-3 sm:pl-0 whitespace-pre-line">
                <p className="text-xs font-bold text-[#002147] tracking-wide whitespace-pre-line">{block.documentNoPrefix || "BELGE NO: "}{docNo}</p>
                <p className="text-xs text-slate-500 whitespace-pre-line">Tarih: {formattedDocDate}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-12 mb-10 bg-slate-50/80 p-6 rounded-xl border border-slate-200/80 whitespace-pre-line">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-pre-line">Adı Soyadı</p>
                <p className="font-bold text-[#002147] text-lg uppercase whitespace-pre-line">{studentName}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-pre-line">T.C. Kimlik No</p>
                <p className="font-bold text-[#002147] text-lg whitespace-pre-line">{studentTc}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-pre-line">Sınıf Seviyesi</p>
                <p className="font-bold text-[#002147] text-lg whitespace-pre-line">{formattedGrade}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-pre-line">Sınav Tarihi</p>
                <p className="font-bold text-[#002147] text-lg whitespace-pre-line">{examDate}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-pre-line">Sınav Saati</p>
                <p className="font-bold text-[#002147] text-lg whitespace-pre-line">{examTime}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-pre-line">Sınav Merkezi</p>
                <p className="font-bold text-[#002147] text-lg whitespace-pre-line">{campusName}</p>
              </div>
            </div>
            ...
            <div className="mt-auto border-t border-slate-200 pt-8 grid grid-cols-1 md:grid-cols-3 gap-6 text-left whitespace-pre-line">
              <div>
                <h4 className="text-xs font-bold text-[#002147] mb-2 uppercase tracking-wider flex items-center gap-1.5 whitespace-pre-line">
                  <span className="material-symbols-outlined text-sm text-primary whitespace-pre-line" translate="no" aria-hidden="true">gavel</span>
                  Sınav Kuralları
                </h4>
              ...
              <div>
                <h4 className="text-xs font-bold text-[#002147] mb-2 uppercase tracking-wider flex items-center gap-1.5 whitespace-pre-line">
                  <span className="material-symbols-outlined text-sm text-primary whitespace-pre-line" translate="no" aria-hidden="true">badge</span>
                  Gerekli Belgeler
                </h4>
                <ul className="text-[11px] text-slate-600 space-y-1.5 list-disc pl-4 leading-relaxed whitespace-pre-line">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#002147] mb-2 uppercase tracking-wider flex items-center gap-1.5 whitespace-pre-line">
                  <span className="material-symbols-outlined text-sm text-primary whitespace-pre-line" translate="no" aria-hidden="true">location_on</span>
                  Kampüs Adresi
                </h4>
            ...
            <div className="mt-8 pt-4 border-t border-slate-100 flex justify-between items-center text-[10px] text-slate-400 whitespace-pre-line">
              ...
              <span className="font-mono whitespace-pre-line">VERIFIED OFFICIAL ENTRY TICKET</span>
            </div>
          </div>
        </div>

        {/* Secondary Action Link */}
        <div className={`mt-10 ${block.styles?.textAlign ? "" : "text-center"} no-print`}>
          <a href="/" className="text-primary font-bold inline-flex items-center justify-center gap-2 hover:underline text-sm whitespace-pre-line">
            <span className="material-symbols-outlined text-lg whitespace-pre-line" translate="no" aria-hidden="true">home</span> Ana Sayfaya Dön
          </a>
        </div>
...
export const DynamicBlockRenderer = ({
  ...
  const getStyle = (block: any, prefix: string) => {
    return {
      ...
      textAlign: block.styles?.[prefix + "Align"] || block.styles?.textAlign || undefined,
  ...
  const getDescStyle = (block: any) => ({ ...getStyle(block, "desc"), whiteSpace: "pre-line" as const });
  ...
  const renderBlock = (block: any, index: number) => {
    const renderContent = () => {
      ...
      case 'campus_hero':
        return (
          <section key={index} className="relative h-[600px] w-full flex items-center overflow-hidden transition-all duration-1000 opacity-100 translate-y-0 whitespace-pre-line" style={getStyle(block, 'container')}>
            <div className="absolute inset-0 z-0 whitespace-pre-line">
              <div className="absolute inset-0 bg-gradient-to-r from-primary/80 to-transparent z-10 whitespace-pre-line"></div>
              {block.image && <img className="w-full h-full object-cover whitespace-pre-line" src={block.image} alt="" style={{...getImageStyle(block, 'image'), backgroundSize: 'cover', backgroundPosition: 'center center'}} />}
            </div>
            <div className={`relative z-20 ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop w-full`}>
              <div className={getHeroInnerClass(block, "max-w-2xl")}>
                <h1 className="font-display-lg text-display-lg text-white mb-6 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h1>
                <p className="font-body-md text-body-lg text-white/90 mb-8 leading-relaxed whitespace-pre-line" style={getSubtitleStyle(block)}>
                  {block.subtitle}
                </p>
                {block.buttons && block.buttons.length > 0 && (
                  <div className="flex gap-4 flex-wrap whitespace-pre-line">
                    {block.buttons.map((btn: any, btnIdx: number) => (
                      <SmartLink key={btnIdx} url={btn.url || btn.buttonUrl || btn.link} className="bg-white text-primary px-8 py-4 rounded-xl font-bold hover:bg-surface-container-low transition-all shadow-lg flex items-center gap-2 whitespace-pre-line" style={getIndividualButtonStyle(btn)}>
      ...
      case 'campus_bento':
        ...
        return (
          <section key={index} className={`py-section-gap ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop transition-all duration-1000 opacity-100 translate-y-0`} style={getStyle(block, 'container')}>
            <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-16`}>
              <h2 className="font-headline-xl text-headline-xl text-text-main mb-4 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
              {block.subtitle && (
                <p className="font-body-lg text-text-muted max-w-2xl mx-auto mb-4 whitespace-pre-line" style={getSubtitleStyle(block)} dangerouslySetInnerHTML={{ __html: block.subtitle }} />
              )}
            ...
            <div className="grid grid-cols-12 gap-6 whitespace-pre-line">
              {(block.items || []).map((item: any, i: number) => {
                const cols = item.colSpan || 'col-span-12 md:col-span-6 lg:col-span-4';
                const baseStyle = getCardStyle(item);
                if (!item.image && !baseStyle.backgroundColor && !item.styles?.backgroundColor) {
                    baseStyle.backgroundColor = '#747685';
                } else if (item.styles?.backgroundColor) {
                    baseStyle.backgroundColor = item.styles.backgroundColor;
                }
                return (
                  <div key={i} className={`${cols} group relative overflow-hidden rounded-3xl h-[400px] border border-border-subtle hover:border-primary transition-all duration-500 shadow-sm`} style={baseStyle}>
                    {item.image && (
                      <img className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 whitespace-pre-line" src={item.image} alt="" style={getImageStyle(item, 'image', i)} />
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-8 whitespace-pre-line">
                      <h3 className="font-headline-md text-headline-md text-white mb-2 whitespace-pre-line" style={getCardTitleStyle(item, block)}>{item.title}</h3>
                      {item.desc && <p className="text-white/80 font-body-md whitespace-pre-line" style={getCardDescStyle(item, block)}>{item.desc}</p>}
                    </div>
                  </div>
                );
              })}
      ...
      case 'campus_gallery':
        return (
          <section key={index} className="bg-surface-container-low py-section-gap transition-all duration-1000 opacity-100 translate-y-0 whitespace-pre-line" style={getStyle(block, 'container')}>
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
              <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 whitespace-pre-line">
                <div>
                  <h2 className="font-headline-xl text-headline-xl text-text-main mb-4 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
                  <p className="font-body-md text-text-muted max-w-xl whitespace-pre-line" style={getSubtitleStyle(block)}>{block.subtitle}</p>
                </div>
                <div className="flex gap-2 whitespace-pre-line">
                  <button className="w-12 h-12 rounded-full border border-primary text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all whitespace-pre-line" onClick={(e) => { const c = e.currentTarget.parentElement?.parentElement?.nextElementSibling; if (c) { c.scrollBy({ left: -350, behavior: 'smooth' }); } }}>
                    <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">chevron_left</span>
                  </button>
                  <button className="w-12 h-12 rounded-full border border-primary text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all whitespace-pre-line" onClick={(e) => { const c = e.currentTarget.parentElement?.parentElement?.nextElementSibling; if (c) { c.scrollBy({ left: 350, behavior: 'smooth' }); } }}>
                    <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">chevron_right</span>
                  </button>
                </div>
              </div>
              <div className="flex gap-6 overflow-x-auto no-scrollbar pb-4 whitespace-pre-line" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                {(block.items || []).map((item: any, i: number) => (
                  <div key={i} className="min-w-[350px] bg-white rounded-2xl overflow-hidden shadow-sm border border-border-subtle group flex-shrink-0 whitespace-pre-line" style={getCardStyle(item)}>
                    <div className="h-64 overflow-hidden relative whitespace-pre-line">
                      {item.image && <img className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 whitespace-pre-line" src={item.image} alt="" style={getImageStyle(item, 'image', i)} />}
                    </div>
                    <div className="p-6 whitespace-pre-line">
                      <h4 className="font-headline-md text-[20px] mb-2 whitespace-pre-line" style={getCardTitleStyle(item, block)}>{item.title}</h4>
                      <p className="text-text-muted text-label-md whitespace-pre-line" style={getCardDescStyle(item, block)}>{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="grid grid-cols-2 gap-6 items-center whitespace-pre-line">
                  <div className="flex flex-col gap-6 whitespace-pre-line">
                    <div className="h-48 rounded-2xl overflow-hidden shadow-lg  whitespace-pre-line">
                      {block.image1 && <img className="w-full h-full object-cover whitespace-pre-line" src={block.image1} alt="" />}
                    </div>
                    <div className="h-64 rounded-2xl overflow-hidden shadow-lg whitespace-pre-line">
                      {block.image2 && <img className="w-full h-full object-cover whitespace-pre-line" src={block.image2} alt="" />}
                    </div>
                  </div>
                  <div className="flex flex-col gap-6 pt-12 whitespace-pre-line">
                    <div className="h-64 rounded-2xl overflow-hidden shadow-lg whitespace-pre-line">
                      {block.image3 && <img className="w-full h-full object-cover whitespace-pre-line" src={block.image3} alt="" />}
                    </div>
                    <div className="h-48 rounded-2xl overflow-hidden shadow-lg  whitespace-pre-line">
                      {block.image4 && <img className="w-full h-full object-cover whitespace-pre-line" src={block.image4} alt="" />}
                    </div>
                  </div>
                </div>
              </div>
              <div className="order-1 lg:order-2 space-y-8 whitespace-pre-line">
                {block.badge && <span className="text-primary font-bold text-label-md tracking-widest uppercase whitespace-pre-line" style={getBadgeStyle(block)}>{block.badge}</span>}
                <h2 className="font-headline-xl text-headline-xl text-text-main whitespace-pre-line" style={getTitleStyle(block)}>
                  {block.titlePart1} {block.titlePart2 && <div style={getTitlePart2Style(block)}>{block.titlePart2}</div>}
                </h2>
                <p className="font-body-md text-text-muted leading-relaxed whitespace-pre-line" style={getSubtitleStyle(block)}>
                  {block.subtitle}
                </p>
                <ul className="space-y-4 whitespace-pre-line">
                  {(block.items || []).map((item: any, i: number) => (
                    <li key={i} className="flex items-center gap-4 whitespace-pre-line">
                      <div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center flex-shrink-0 whitespace-pre-line">
                        {item.icon && (typeof item.icon === 'string' && item.icon === item.icon.toLowerCase() ? <IconPreview data={item.icon}  /> : <IconPreview data={item.icon} className="w-6 h-6 fill-current whitespace-pre-line" />)}
                      </div>
                      <span className="font-label-md text-text-main whitespace-pre-line" style={getCardTitleStyle(item, block)}>{item.title}</span>
                    </li>
                  ))}
                </ul>
                {block.buttons && block.buttons.length > 0 && (
      ...
      case 'campus_contact':
        return (
          <section key={index} className="py-section-gap bg-surface-dim/30 transition-all duration-1000 opacity-100 translate-y-0 whitespace-pre-line" style={getStyle(block, 'container')}>
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
              <div className="bg-white rounded-[32px] overflow-hidden shadow-2xl flex flex-col lg:flex-row whitespace-pre-line">
                <div className="p-12 lg:w-1/2 space-y-8 flex flex-col justify-center whitespace-pre-line">
                  <h2 className="font-headline-xl text-headline-xl whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
                  {block.subtitle && (
                    <p className="font-body-lg text-text-muted whitespace-pre-line" style={getSubtitleStyle(block)} dangerouslySetInnerHTML={{ __html: block.subtitle }} />
                  )}
                  <div className="space-y-6 whitespace-pre-line">
                    {(block.items || []).map((item: any, i: number) => (
                      <div key={i} className="flex gap-4 items-start whitespace-pre-line">
                        <IconPreview data={item.icon} className="text-primary whitespace-pre-line" />
                        <div>
                          <h4 className="font-bold text-text-main whitespace-pre-line" style={getCardTitleStyle(item, block)}>{item.title}</h4>
                          <p className="text-text-muted whitespace-pre-line" style={getCardDescStyle(item, block)}>{item.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="lg:w-1/2 min-h-[400px] relative overflow-hidden whitespace-pre-line">
                  {block.mapCode ? (
                    ...
                    <>
                      {block.image && <img className="absolute inset-0 w-full h-full object-cover whitespace-pre-line" src={block.image} alt="" style={getImageStyle(block, 'image')} />}
                      <div className="absolute bottom-8 left-8 right-8 bg-white/80 backdrop-blur-md p-6 rounded-2xl flex items-center justify-between border border-border-subtle/80 whitespace-pre-line">
                        <div>
                          <h4 className="font-bold whitespace-pre-line">{block.cardTitle}</h4>
                          <p className="text-sm text-text-muted whitespace-pre-line">{block.cardDesc}</p>
                        </div>
                        {block.buttons && block.buttons[0] && (
                          <SmartLink url={block.buttons[0].url || block.buttons[0].buttonUrl || block.buttons[0].link} className="bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center hover:opacity-90 transition-opacity whitespace-pre-line">
        ...
        case "contact_hero":
          return (
            <section
              ...
              style={getStyle(block, "container")}
            >
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop ${block.styles?.textAlign ? "" : "text-center"}`}>
                <h1
                    ...
                    >
                      <div className="md:col-span-1 p-8 flex flex-col justify-center border-r border-border-subtle whitespace-pre-line">
                        ...
                        <div className="space-y-4 mb-6 whitespace-pre-line">
                          {item.address && (
                            <div className="flex items-start gap-3 whitespace-pre-line">
                              <span className="material-symbols-outlined text-primary text-[24px] whitespace-pre-line" translate="no" aria-hidden="true">
                                location_on
                              </span>
                        ...
                        {item.buttonText && (
                          ...
                          >
                            <span className="material-symbols-outlined text-[20px] whitespace-pre-line" translate="no" aria-hidden="true">
                              directions
                            </span>{" "}
                            {item.buttonText}
                          </a>
                    ...
                    {validItems.map((item: any, i: number) => (
                      ...
                      >
                        {item.icon &&
                          (typeof item.icon === "string" &&
                          item.icon === item.icon.toLowerCase() ? (
                            <IconPreview data={item.icon} className="text-2xl whitespace-pre-line" />
                          ) : (
                            <IconPreview
                              data={item.icon}
                    ...
                    >
                      <div className="w-16 h-16 bg-primary-fixed rounded-2xl flex items-center justify-center mx-auto mb-6 whitespace-pre-line">
                        {item.icon &&
                          (typeof item.icon === "string" &&
                          item.icon === item.icon.toLowerCase() ? (
                            <IconPreview data={item.icon} className="text-primary text-3xl whitespace-pre-line" />
                          ) : (
                            <IconPreview
                              data={item.icon}
            ...
            >
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-gutter`}>
                <div className="bg-primary rounded-3xl p-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 shadow-md whitespace-pre-line">
                  <div className={`relative z-10 ${block.styles?.textAlign ? "" : "text-center"} md:text-left`}>
                    <h2
        ...
        case "news_hero":
          return (
            ...
            >
              ...
              >
                <div className={getHeroInnerClass(block, "max-w-2xl text-on-primary")}>
                  {block.title && (
                    <h1
                      style={getTitleStyle(block)}
                      className="font-display-lg text-[36px] md:text-display-lg mb-4 whitespace-pre-line"
                      dangerouslySetInnerHTML={{ __html: (block.title || "").replace("Eğitimde Dostluk, Gelecekte Başarı", "Eğitimde Dostluk,<br />Gelecekte Başarı") }}
                    ></h1>
                  )}
                  {block.subtitle && (
                    <p
              ...
              >
                      ...
                      >
                        {item.date && (
                          <div className="flex items-center gap-2 text-text-muted mb-3 font-label-sm text-xs md:text-label-sm whitespace-pre-line">
                            <span className="material-symbols-outlined text-[16px] md:text-[18px] whitespace-pre-line" translate="no" aria-hidden="true">
                              calendar_today
                            </span>
                            <span>{item.date}</span>
                          </div>
                        ...
                        <div className="mt-auto whitespace-pre-line">
                          ...
                          >
                            ...
                            <span className="material-symbols-outlined transition-transform group-hover/link:translate-x-1 text-sm md:text-base whitespace-pre-line" translate="no" aria-hidden="true">
                              arrow_forward
                            </span>
                          </a>
                        </div>
                ...
                {block.showPagination && (
                  <div className="mt-12 md:mt-16 flex justify-center items-center gap-2 whitespace-pre-line">
                    ...
                    <span className="px-2 text-text-muted whitespace-pre-line">...</span>
                    <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-border-subtle text-on-surface-variant hover:bg-surface-container-high transition-colors font-label-md whitespace-pre-line">
                      12
                    </button>
                    <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-border-subtle text-on-surface-variant hover:bg-surface-container-high transition-colors whitespace-pre-line">
        ...
        case "newsletter":
          return (
            ...
            >
              ...
              >
                {block.icon && (
                  <IconPreview data={block.icon} className="text-primary text-2xl md:text-4xl mb-4 whitespace-pre-line" style={{ fontVariationSettings: "'FILL' 1"  }} />
                )}
                {block.title && (
                  <h2
                    style={getTitleStyle(block)}
              ...
              <div className="w-full flex flex-col gap-6 md:gap-10 whitespace-pre-line">
                {(() => {
                        ...
                        return (
                          <h1
                            ...
                            dangerouslySetInnerHTML={{
                              __html: (block.title || "").replace("Eğitimde Dostluk, Gelecekte Başarı", "Eğitimde Dostluk,<br />Gelecekte Başarı"),
                            }}
                          />
                        );
                      })()}
                  ...
                  const imagesContent = (
                    <div className="w-full whitespace-pre-line">
                      {(() => {
                        ...
                        return (
                          <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto w-full flex flex-col md:flex-row gap-3 md:gap-6 relative z-10 px-4 md:px-0 h-[560px] md:h-[480px]`}>
                  ...
                  const buttonsContent =
                    block.buttons && block.buttons.length > 0 ? (
                      <div className={`flex flex-col sm:flex-row gap-4 justify-center items-center w-full ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto relative z-10`}>
                        {block.buttons.map((btn: any, i: number) => {
                            ...
                            >
                              ...
                              {btn.icon &&
                                ...
                                btn.icon === btn.icon.toLowerCase() ? (
                                  <IconPreview data={btn.icon} className="text-[1.1em] whitespace-pre-line" />
                                ) : (
                                  <IconPreview
                                    data={btn.icon}
              ...
              >
                <div className="flex flex-row items-center gap-4 mb-10 md:mb-12 whitespace-pre-line">
                  <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center shrink-0 shadow-lg backdrop-blur-sm whitespace-pre-line">
                    <span className="material-symbols-outlined text-white text-3xl whitespace-pre-line" translate="no" aria-hidden="true">
                      school
                    </span>
                  </div>
                  <div>
                    ...
                    >
                      <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 shadow-inner whitespace-pre-line">
                        ...
                        (typeof item.icon === "string" &&
                          ...
                          <IconPreview data={item.icon || "school"} className="text-[#5eead4] whitespace-pre-line" />
                        )}
                      </div>
                      <h3
                        style={getCardTitleStyle(item, block)}
        ...
        case "features":
          return (
            ...
            >
              ...
              >
                <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-10 md:mb-16`}>
                  <span
                    style={getSubtitleStyle(block)}
                    ...
                    >
                      {item.highlight ? (
                        <>
                          {item.image && (
                            <div className="absolute inset-0 z-0 whitespace-pre-line">
                              <img src={item.image} alt={item.title} className="w-full h-full object-cover opacity-40 mix-blend-overlay whitespace-pre-line" />
                            </div>
                          )}
                          <div className={`relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 ${block.styles?.textAlign ? "" : "text-center"} md:text-left`}>
                            <div className="flex-1 whitespace-pre-line">
                              ...
                              {item.buttonText && (
                                ...
                                >
                                  {item.buttonText}{" "}
                                  {item.buttonIcon && (
                                    <span className="material-symbols-outlined ml-1 text-[0.9em] whitespace-pre-line" translate="no" aria-hidden="true">
                                      {item.buttonIcon}
                                    </span>
                                  )}
                                </SmartLink>
                            ...
                            <div className="w-full h-48 md:h-64 rounded-xl overflow-hidden mb-5 shrink-0 relative whitespace-pre-line">
                              ...
                              <div className="absolute top-4 left-4 w-10 h-10 rounded-xl bg-white/90 backdrop-blur-sm text-primary flex items-center justify-center shadow-sm whitespace-pre-line">
                                ...
                                (typeof item.icon === "string" &&
                                  ...
                                  <IconPreview data={item.icon} className="text-xl whitespace-pre-line" />
                                )}
                              </div>
                            </div>
                          )}
                    ...
                    >
                      ...
                      <div className="p-5 md:p-6 flex flex-col flex-1 whitespace-pre-line">
                        <div className="flex items-center justify-between mb-3 md:mb-4 whitespace-pre-line">
                          ...
                          <span className="text-primary material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">
                            location_on
                          </span>
                        </div>
                        <p
                  ...
                  <div className="lg:col-span-7 group relative aspect-video bg-inverse-surface rounded-2xl overflow-hidden shadow-2xl border border-border-subtle cursor-pointer transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(29,78,202,0.2)] whitespace-pre-line">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center whitespace-pre-line">
                      <div className="relative whitespace-pre-line">
                        ...
                        >
                          <span
                            className="material-symbols-outlined text-2xl md:text-4xl whitespace-pre-line" translate="no" aria-hidden="true"
        ...
        case "achievements_hero":
          return (
            ...
            >
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop grid grid-cols-1 gap-6 relative z-10 py-20 items-center`}>
                <div className={getHeroInnerClass(block, `space-y-6 ${block.styles?.textAlign ? "" : "text-center"} mx-auto`)}>
                  {block.badge && (
                  ...
                  {block.titlePart1 || block.titlePart2 ? (
                    <h1 className="font-display-lg text-display-lg leading-tight whitespace-pre-line" style={getTitleStyle(block)}>
                      {block.titlePart1}
                      {block.titlePart1 && block.titlePart2 && " "}
                      {block.titlePart2 && (
                        <span
                    ...
                    <h1 className="font-display-lg text-display-lg leading-tight whitespace-pre-line" style={getTitleStyle(block)} dangerouslySetInnerHTML={{ __html: (block.title || "").replace("Eğitimde Dostluk, Gelecekte Başarı", "Eğitimde Dostluk,<br />Gelecekte Başarı") }}></h1>
                  )}
                  {block.subtitle && (
                    <p
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 whitespace-pre-line">
                  ...
                  {block.items && block.items[0] && (
                    <div className="bg-surface-card border border-border-subtle rounded-3xl p-8 bento-card relative overflow-hidden transition-all duration-700 hover:-translate-y-2 whitespace-pre-line">
                      ...
                      ></div>
                      <IconPreview data={block.items[0].icon || "school"} className="text-primary text-4xl mb-6 whitespace-pre-line" />
                  ...
                  {block.items && block.items[1] && (
                    <div className="bg-primary text-on-primary rounded-3xl p-8 bento-card relative overflow-hidden transition-all duration-700 hover:-translate-y-2 whitespace-pre-line">
                      ...
                      <IconPreview data={block.items[1].icon || "star"} className="text-gold text-4xl mb-6 whitespace-pre-line" style={{ fontVariationSettings: "'FILL' 1",
                          color: "#D4AF37",
                         }} />
                  ...
                  {block.items && block.items[2] && (
                    <div className="bg-surface-card border border-border-subtle rounded-3xl p-8 bento-card flex flex-col justify-between transition-all duration-700 hover:-translate-y-2 whitespace-pre-line">
                      <div>
                        <IconPreview data={block.items[2].icon || "analytics"} className="text-primary text-4xl mb-6 whitespace-pre-line" />
                        <h3 className="font-headline-md text-headline-md mb-2 whitespace-pre-line">
                          {block.items[2].title}
                        </h3>
                        ...
                        {block.items[2].list?.map(
                          (listItem: string, i: number) => (
                            <li key={i} className="flex items-center gap-3 whitespace-pre-line">
                              ...
                              >
                                <span
                                  className="material-symbols-outlined text-gold text-lg whitespace-pre-line" translate="no" aria-hidden="true"
                                  style={{ color: "#D4AF37" }}
                                >
                                  check_circle
                                </span>
                              </div>
                              <span className="text-label-md whitespace-pre-line">{listItem}</span>
                            </li>
                          ),
                        )}
                      </ul>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 whitespace-pre-line">
                  {block.items?.map((item: any, i: number) => (
                    ...
                    >
                      ...
                      <h4 className="font-bold text-lg mb-1 whitespace-pre-line">{item.title}</h4>
                      ...
                      <div className="space-y-6 whitespace-pre-line">
                        {block.items?.map((item: any, i: number) => (
                          <div
                            ...
                            style={getCardStyle(item)}
                          >
                            <IconPreview data={item.icon || "science"} className="text-gold text-3xl whitespace-pre-line" style={{ color: "#D4AF37"  }} />
                            <div>
                              <h4 className="font-bold mb-1 whitespace-pre-line">{item.title}</h4>
                              <p className="text-sm opacity-70 whitespace-pre-line">{item.desc}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <div>
                  ...
                  {block.buttonText && (
                    ...
                    >
                      {block.buttonText}
                      {block.buttonIcon && (
                        <span className="material-symbols-outlined text-[1.1em] whitespace-pre-line" translate="no" aria-hidden="true">
                          {block.buttonIcon}
                        </span>
                      )}
                    </SmartLink>
                ...
                <div className="flex justify-between items-end mb-10 md:mb-12 whitespace-pre-line">
                  ...
                  <div className="flex gap-2 md:gap-4 whitespace-pre-line">
                    <button className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-border-subtle flex items-center justify-center hover:bg-primary/5 transition-colors whitespace-pre-line">
                      <span className="material-symbols-outlined text-xl md:text-2xl whitespace-pre-line" translate="no" aria-hidden="true">
                        chevron_left
                      </span>
                    </button>
                    <button className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-border-subtle flex items-center justify-center hover:bg-primary/5 transition-colors whitespace-pre-line">
        ...
        case "about_hero":
          return (
            ...
            >
              ...
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop relative z-20 w-full`}>
              ...
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
                {(block.title || block.subtitle) && (
                  <div className={`mb-12 ${block.styles?.textAlign ? "" : "text-center"}`}>
                    {block.title && <h2 className="font-headline-xl text-headline-xl text-text-main mb-4 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>}
                    {block.subtitle && <p className="font-body-lg text-text-muted max-w-2xl mx-auto whitespace-pre-line" style={getSubtitleStyle(block)} dangerouslySetInnerHTML={{ __html: block.subtitle }} />}
                  </div>
                )}
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        (typeof item.icon === "string" &&
                          item.icon !== item.icon.toLowerCase()) ? (
                          <IconPreview data={item.icon} className="w-10 h-10 whitespace-pre-line" />
                        ) : (
                          <IconPreview data={item.icon || (isSecondary ? "visibility" : "flag")} className="text-4xl whitespace-pre-line" />
                        )}
                      </div>
                      <h2
                        style={getCardTitleStyle(item, block)}
              ...
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
                <div className="bg-surface-card rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-xl whitespace-pre-line">
                  ...
                  <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center relative whitespace-pre-line">
                    <span className="material-symbols-outlined text-primary-fixed text-8xl absolute top-4 md:top-8 left-4 md:left-8 opacity-40 whitespace-pre-line" translate="no" aria-hidden="true">
                      format_quote
                    </span>
                    <div className="relative z-10 whitespace-pre-line">
                      {block.title && (
                      ...
                      <div>
                        ...
                        <p className="text-text-muted whitespace-pre-line">{block.authorTitle}</p>
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        (item: any, i: number) => (
                          ...
                          >
                            ...
                            (typeof item.icon === "string" &&
                              ...
                              <IconPreview data={item.icon || "school"} className="text-[20px] whitespace-pre-line" style={{ color: "inherit"  }} />
                            )}
        ...
        case "management_hero":
          return (
            ...
            >
              ...
              ></div>
              <div className={`relative z-10 layout-content-container w-full max-w-[1200px] px-6 ${block.styles?.textAlign ? "" : "text-center"} md:text-left`}>
                <h1
              ...
              <div className="mb-12 whitespace-pre-line">
                <div className="flex items-center gap-3 mb-6 border-b border-[#bd0f2c]/20 pb-2 whitespace-pre-line">
                  ...
                  (typeof block.icon === "string" &&
                    ...
                    <IconPreview data={block.icon || "school"} className="text-[#bd0f2c] whitespace-pre-line" />
                  )}
                  <h2
                ...
                <div className="group relative flex flex-col md:flex-row items-stretch gap-0 rounded-xl bg-white dark:bg-background-dark shadow-xl overflow-hidden border border-slate-100 dark:border-slate-800 whitespace-pre-line">
                  ...
                  <div className="md:w-3/5 flex flex-col justify-center p-8 md:p-12 gap-6 bg-gradient-to-br from-white to-slate-50 dark:from-background-dark dark:to-slate-900 whitespace-pre-line">
                    ...
                    <div className="relative whitespace-pre-line">
                      <span className="material-symbols-outlined absolute -top-4 -left-6 text-slate-200 dark:text-slate-800 text-6xl select-none whitespace-pre-line" translate="no" aria-hidden="true">
                        format_quote
                      </span>
                      <p
                        ...
                        return (
                          ...
                          >
                            ...
                            {btn.icon &&
                              (typeof btn.icon === "string" &&
                              btn.icon === btn.icon.toLowerCase() ? (
                                <IconPreview data={btn.icon} className="text-[18px] whitespace-pre-line" />
                              ) : (
                                <IconPreview
                                  data={btn.icon}
              ...
              <div className="mb-12 whitespace-pre-line">
                <div className="flex items-center gap-3 mb-8 border-b border-[#bd0f2c]/20 pb-2 whitespace-pre-line">
                  ...
                  (typeof block.icon === "string" &&
                    ...
                    <IconPreview data={block.icon || "groups"} className="text-[#bd0f2c] whitespace-pre-line" />
                  )}
                  <h2
                    className="text-2xl font-bold text-slate-900 dark:text-white whitespace-pre-line"
                    style={getTitleStyle(block)}
                      ...
                      <div className="p-6 flex flex-col gap-3 whitespace-pre-line">
                        ...
                        {item.url && (
                          ...
                          >
                            ...
                            <span className="material-symbols-outlined text-[16px] whitespace-pre-line" translate="no" aria-hidden="true">
                              trending_flat
                            </span>
                          </a>
                        )}
                ...
                <div className="flex items-center justify-between mb-8 border-b border-[#bd0f2c]/20 pb-2 whitespace-pre-line">
                  <div className="flex items-center gap-3 whitespace-pre-line">
                    ...
                    (typeof block.icon === "string" &&
                      ...
                      <IconPreview data={block.icon || "account_balance"} className="text-[#bd0f2c] whitespace-pre-line" />
                    )}
                    <h2
                      className="text-2xl font-bold text-slate-900 dark:text-white whitespace-pre-line"
                      style={getTitleStyle(block)}
                      ...
                      {item.url && (
                        <div className="mt-4 pt-3 border-t border-slate-50 dark:border-slate-800 whitespace-pre-line">
                          ...
                          >
                            ...
                            <span className="material-symbols-outlined text-[14px] whitespace-pre-line" translate="no" aria-hidden="true">
                              open_in_new
                            </span>
                          </a>
                        </div>
        ...
        case "kindergarten_hero":
          return (
            <section
              ...
              style={getStyle(block, "container")}
            >
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 items-center relative z-10`}>
                <div className="space-y-6 whitespace-pre-line">
                  {block.badge && (
                  ...
                  {(block.imageBadgeTitle || block.imageBadgeDesc) && (
                    <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end whitespace-pre-line">
                      <div className="bg-surface/90 backdrop-blur-sm p-4 rounded-lg border border-border-subtle whitespace-pre-line">
                        <div className="flex items-center gap-3 whitespace-pre-line">
                          <span className="material-symbols-outlined text-secondary text-[32px] whitespace-pre-line" translate="no" aria-hidden="true">
        ...
        case "kindergarten_bento":
          return (
            <section
              ...
              style={getStyle(block, "container")}
            >
              <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto`}>
                <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-16 max-w-2xl mx-auto`}>
                  <h2
                          ...
                          <div className="relative z-10 flex flex-col h-full justify-between whitespace-pre-line">
                            <div>
                              ...
                              (typeof item.icon === "string" &&
                                ...
                                <IconPreview data={item.icon || "forest"} className="text-secondary-fixed text-[40px] mb-4 whitespace-pre-line" />
                              )}
                              <h3
                            ...
                            {item.url && !item.hideButton && (
                              ...
                              >
                                ...
                                <span className="material-symbols-outlined text-[18px] whitespace-pre-line" translate="no" aria-hidden="true">
                                  arrow_forward
                                </span>
                              </a>
                            )}
                        ...
                        >
                          <div className="relative z-10 whitespace-pre-line">
                            ...
                            (typeof item.icon === "string" &&
                              ...
                              <IconPreview data={item.icon || "psychology"} className="text-primary text-[40px] mb-4 whitespace-pre-line" />
                            )}
                            <h3
                      ...
                      >
                        <div>
                          ...
                          (typeof item.icon === "string" &&
                            ...
                            <IconPreview data={item.icon || "favorite"} className="text-secondary text-[32px] mb-4 whitespace-pre-line" />
                          )}
                          <h3
                      ...
                      <div className="p-6 whitespace-pre-line">
                        <div className="flex items-center gap-3 mb-3 whitespace-pre-line">
                          ...
                          (typeof item.icon === "string" &&
                            ...
                            <IconPreview data={item.icon || "language"} className="text-primary whitespace-pre-line" />
                          )}
                          <h3
        ...
        case "primary_school_hero":
          return (
            ...
            >
              ...
              <div className={`relative z-10 ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop w-full grid grid-cols-1 md:grid-cols-12 gap-6 items-center`}>
                <div className="col-span-1 md:col-span-7 lg:col-span-6 space-y-6 whitespace-pre-line">
                  {block.badge && (
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full border border-primary/20 whitespace-pre-line">
                      <span
                        className="material-symbols-outlined text-primary text-sm whitespace-pre-line" translate="no" aria-hidden="true"
                        style={{ fontVariationSettings: "'FILL' 1" }}
                      >
                        star
                      </span>
                          ...
                          <div className="relative z-10 whitespace-pre-line">
                            <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center mb-6 whitespace-pre-line">
                              ...
                              (typeof item.icon === "string" &&
                                ...
                                <IconPreview data={item.icon || "menu_book"} className="text-primary whitespace-pre-line" style={{ fontVariationSettings: "'FILL' 1"  }} />
                              )}
                            </div>
                            <h3
                              className="font-headline-md text-headline-md text-on-surface mb-2 whitespace-pre-line"
      ...
      if (block.type === "middle_school_hero") {
        return (
          ...
          >
            ...
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop relative z-10 flex flex-col items-start w-full md:w-2/3`}>
              {block.badge && (
                        ...
                        >
                          ...
                          >
                            ...
                            (typeof item.icon === "string" &&
                              ...
                              <IconPreview data={item.icon || "psychology"} className="text-[28px] whitespace-pre-line" style={{ fontVariationSettings: "'FILL' 1"  }} />
                            )}
                          </div>
                          <h3
      ...
      if (block.type === "middle_school_lgs") {
        return (
          ...
          >
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
              <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-16 max-w-3xl mx-auto`}>
                <h2
                  className="font-headline-xl text-headline-xl text-on-surface mb-4 whitespace-pre-line"
                  style={getTitleStyle(block)}
                >
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        (typeof item.icon === "string" &&
                          item.icon !== item.icon.toLowerCase()) ? (
                          <IconPreview data={item.icon} className="w-6 h-6 whitespace-pre-line" />
                        ) : (
                          <IconPreview data={item.icon || "menu_book"} className="text-[24px] whitespace-pre-line" />
                        )}
                      </div>
                      <div>
                        <h3
      ...
      if (block.type === "high_school_hero") {
        return (
          <section
            ...
            style={getStyle(block, "container")}
          >
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop grid md:grid-cols-2 gap-12 items-center`}>
              <div className="z-10 flex flex-col gap-6 whitespace-pre-line">
                {block.badge && (
                  <div className="inline-flex items-center gap-2 bg-primary-fixed/20 text-primary-container px-3 py-1 rounded-full w-fit whitespace-pre-line">
                    <span className="material-symbols-outlined text-sm whitespace-pre-line" translate="no" aria-hidden="true">
                      stars
                    </span>
      ...
      if (block.type === "achievements_hero") {
        return (
          ...
          >
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-7xl"} mx-auto px-8 grid grid-cols-1 gap-6 relative z-10 py-20 items-center w-full`}>
              <div className={`space-y-6 ${block.styles?.textAlign ? "" : "text-center"} mx-auto`}>
                {block.badge && (
                ...
                <h1 className="font-bold text-2xl md:text-4xl leading-tight tracking-[-0.02em] whitespace-pre-line" style={getTitleStyle(block)}>
                  {block.titlePart1}{" "}
                  {block.titlePart2 && (
                ...
                {block.subtitle && (
                  <p className="text-lg md:text-[18px] text-[#dce1ff] max-w-xl mx-auto whitespace-pre-line" style={getSubtitleStyle(block)}>
                    {block.subtitle}
                  </p>
                )}
      ...
      if (block.type === "achievements_academic_bento") {
        return (
          <section
            ...
            style={getStyle(block, "")}
          >
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-7xl"} mx-auto px-8`}>
              <div className={`${block.styles?.textAlign ? "" : "text-center"} mb-16`}>
                      ...
                      >
                        ...
                        <div className="mb-6 whitespace-pre-line">
                          ...
                          (typeof item.icon === "string" &&
                            ...
                            <IconPreview data={item.icon || "star"} className="text-[#D4AF37] text-4xl whitespace-pre-line" style={{ fontVariationSettings: "'FILL' 1"  }} />
                          )}
                        </div>
                        <h3 className="font-bold text-[24px] mb-2 text-white whitespace-pre-line">
                          {item.title}
                        </h3>
                        <p className="text-[#dce1ff] mb-8 whitespace-pre-line" style={getCardDescStyle(item, block)}>{item.desc}</p>
                        ...
                        <div>
                          <div className="mb-6 whitespace-pre-line">
                            ...
                            (typeof item.icon === "string" &&
                              ...
                              <IconPreview data={item.icon || "analytics"} className="text-[#1d4eca] text-4xl whitespace-pre-line" />
                            )}
                          </div>
                          <h3 className="font-bold text-[24px] mb-2 whitespace-pre-line">
                            {item.title}
                          </h3>
                          <p className="text-[#434654] mb-6 whitespace-pre-line" style={getCardDescStyle(item, block)}>{item.desc}</p>
                        </div>
                            ...
                            {item.listItems.map((li: any, i: number) => (
                              <li key={i} className="flex items-center gap-3 whitespace-pre-line">
                                <div className="w-8 h-8 rounded-full bg-[#D4AF37]/10 flex items-center justify-center shrink-0 whitespace-pre-line">
                                  <span className="material-symbols-outlined text-[#D4AF37] text-lg whitespace-pre-line" translate="no" aria-hidden="true">
                                    check_circle
                                  </span>
                                </div>
                      ...
                      >
                        ...
                        <div className="mb-6 whitespace-pre-line">
                          ...
                          (typeof item.icon === "string" &&
                            ...
                            <IconPreview data={item.icon || "school"} className="text-[#1d4eca] text-4xl whitespace-pre-line" />
                          )}
                        </div>
                        <h3 className="font-bold text-[24px] mb-2 whitespace-pre-line">
                          {item.title}
                        </h3>
                        <p className="text-[#434654] mb-8 whitespace-pre-line" style={getCardDescStyle(item, block)}>{item.desc}</p>
                        <div className="space-y-6 whitespace-pre-line">
                          {item.statValue && (
          ...
          >
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-7xl"} mx-auto px-8`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 whitespace-pre-line">
                {(block.items || []).map((item: any, idx: number) => (
                  <div key={idx} className="group cursor-pointer whitespace-pre-line">
                    <div className="aspect-[3/4] rounded-2xl overflow-hidden mb-4 relative whitespace-pre-line">
                      {item.image ? (
                        <img
                    ...
                    <p className="text-[#434654] text-[12px] font-bold tracking-[0.05em] whitespace-pre-line" style={getCardDescStyle(item, block)}>
                      {item.desc}
                    </p>
                  </div>
                ))}
                        ...
                        >
                          <div className="shrink-0 mt-1 whitespace-pre-line">
                            ...
                            (typeof item.icon === "string" &&
                              ...
                              <IconPreview data={item.icon || "science"} className="text-[#D4AF37] text-3xl whitespace-pre-line" />
                            )}
                          </div>
                          <div>
                            ...
                            <p className="text-sm opacity-70 leading-relaxed whitespace-pre-line" style={getCardDescStyle(item, block)}>
                              {item.desc}
                            </p>
                          </div>
                        </div>
      ...
      if (block.type === 'menu_hero') {
        return (
          <section key={index} className="relative h-[400px] flex items-center overflow-hidden whitespace-pre-line" style={getStyle(block, 'container')}>
            {block.image && <img alt="" className="absolute inset-0 w-full h-full object-cover whitespace-pre-line" src={block.image} style={getImageStyle(block, 'image', index)} />}
            <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-black/30 whitespace-pre-line"></div>
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop relative z-10 w-full`}>
              <div className={getHeroInnerClass(block, "max-w-2xl")}>
                {block.badge && (
                  <span className="inline-block py-1 px-3 rounded-full bg-primary/20 text-[#b6c4ff] text-sm font-semibold mb-4 backdrop-blur-sm border border-white/10 whitespace-pre-line" style={getBadgeStyle(block)}>
                    {block.badge}
                  </span>
                )}
                <h1 className="text-2xl md:text-4xl lg:text-5xl font-black text-white mb-6 leading-tight whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h1>
                {block.subtitle && (
      ...
      if (block.type === 'menu_calendar') {
        return (
          <section key={index} className="py-12 bg-surface-background whitespace-pre-line" style={getStyle(block, 'container')}>
            <div className={`flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10 ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
              <div>
                <h2 className="text-2xl font-bold text-slate-800 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h2>
                {block.subtitle && <p className="text-slate-500 whitespace-pre-line">{block.subtitle}</p>}
              </div>
              <div className="flex flex-wrap items-center gap-4 whitespace-pre-line">
                <div className="flex items-center bg-white rounded-xl border border-slate-200 p-1.5 shadow-sm whitespace-pre-line">
                  <button className="p-2 hover:bg-slate-50 rounded-lg transition-colors whitespace-pre-line">
                    <span className="material-symbols-outlined text-slate-500 whitespace-pre-line" translate="no" aria-hidden="true">chevron_left</span>
                  </button>
                  <span className={`px-6 font-bold text-slate-800 min-w-[140px] ${block.styles?.textAlign ? "" : "text-center"}`}>{block.month || "Ekim 2023"}</span>
                  <button className="p-2 hover:bg-slate-50 rounded-lg transition-colors whitespace-pre-line">
                    <span className="material-symbols-outlined text-slate-500 whitespace-pre-line" translate="no" aria-hidden="true">chevron_right</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`}>
              <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden overflow-x-auto whitespace-pre-line">
                <div className="min-w-[800px] whitespace-pre-line">
                  ...
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest hidden md:block`}>Pazartesi</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest hidden md:block`}>Salı</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest hidden md:block`}>Çarşamba</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest hidden md:block`}>Perşembe</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest hidden md:block`}>Cuma</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-primary uppercase tracking-widest hidden md:block`}>Cumartesi</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-red-500 uppercase tracking-widest hidden md:block`}>Pazar</div>
                  
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest md:hidden`}>Pzt</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest md:hidden`}>Sal</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest md:hidden`}>Çar</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest md:hidden`}>Per</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-slate-600 uppercase tracking-widest md:hidden`}>Cum</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-primary uppercase tracking-widest md:hidden`}>Cmt</div>
                  <div className={`py-5 ${block.styles?.textAlign ? "" : "text-center"} text-xs font-black text-red-500 uppercase tracking-widest md:hidden`}>Paz</div>
                </div>
                ...
                <div className="grid grid-cols-7 divide-x divide-y divide-slate-200 whitespace-pre-line">
                  {block.days?.map((day: any, i: number) => {
                    if (!day.isCurrentMonth) {
                      return (
                        <div key={i} className="p-2 md:p-4 bg-slate-50/50 min-h-[120px] md:min-h-[220px] whitespace-pre-line">
                          <span className="text-slate-400 font-medium opacity-50 text-xs md:text-base whitespace-pre-line">{day.date}</span>
                        </div>
                      );
                    }
                    ...
                    if (day.isClosed) {
                      return (
                        <div key={i} className="p-2 md:p-4 hover:bg-slate-50 transition-colors group min-h-[120px] md:min-h-[220px] bg-red-50/30 whitespace-pre-line">
                          <div className="flex flex-col md:flex-row md:justify-between items-start gap-1 mb-3 whitespace-pre-line">
                            <span className="text-sm md:text-xl font-black text-slate-800 whitespace-pre-line">{day.date}</span>
                            <span className={`text-[8px] md:text-[10px] font-bold px-1.5 md:px-2 py-0.5 md:py-1 rounded-full bg-red-100 text-red-600 uppercase ${block.styles?.textAlign ? "" : "text-center"} w-full md:w-auto`}>Kapalı</span>
                          </div>
                        </div>
                      );
                    }
                    ...
                    if (day.isToday) {
                      return (
                        <div key={i} className="p-2 md:p-4 bg-primary/5 transition-colors ring-1 md:ring-2 ring-inset ring-primary relative min-h-[120px] md:min-h-[220px] whitespace-pre-line">
                          ...
                          <div className="flex flex-col xl:flex-row xl:justify-between items-start gap-1 mb-3 whitespace-pre-line">
                            <span className="text-sm md:text-xl font-black text-primary whitespace-pre-line">{day.date}</span>
                            <span className={`text-[8px] md:text-[10px] font-bold px-1.5 md:px-2 py-0.5 md:py-1 rounded-full bg-primary text-white uppercase ${block.styles?.textAlign ? "" : "text-center"} w-full xl:w-auto`}>Bugün</span>
                          </div>
                          <ul className="space-y-1 md:space-y-2 whitespace-pre-line">
                            {(Array.isArray(day.meals) ? day.meals : typeof day.meals === 'string' ? day.meals.split(/[,\n]+/).map((s: string) => s.trim()).filter(Boolean) : [])?.map((meal: string, mIndex: number) => (
                              <li key={mIndex} className="text-[10px] md:text-sm font-bold text-slate-800 flex items-start gap-1 md:gap-2 leading-tight whitespace-pre-line">
                                <span className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-primary mt-1 md:mt-1.5 shrink-0 whitespace-pre-line"></span> {meal}
                              </li>
                            ))}
                          </ul>
                        </div>
                    ...
                    return (
                      <div key={i} className="p-2 md:p-4 hover:bg-slate-50 transition-colors group min-h-[120px] md:min-h-[220px] whitespace-pre-line">
                        <div className="flex flex-col xl:flex-row xl:justify-between items-start gap-1 mb-3 whitespace-pre-line">
                          <span className="text-sm md:text-xl font-black text-slate-800 whitespace-pre-line">{day.date}</span>
                          {day.kcal && <span className={`text-[8px] md:text-[10px] font-bold px-1.5 md:px-2 py-0.5 md:py-1 rounded-full bg-[#0ea5e9]/10 text-[#0ea5e9] uppercase ${block.styles?.textAlign ? "" : "text-center"} w-full xl:w-auto`}>{day.kcal}</span>}
                        </div>
                        <ul className="space-y-1 md:space-y-2 whitespace-pre-line">
                          {(Array.isArray(day.meals) ? day.meals : typeof day.meals === 'string' ? day.meals.split(/[,\n]+/).map((s: string) => s.trim()).filter(Boolean) : [])?.map((meal: string, mIndex: number) => (
                            <li key={mIndex} className="text-[10px] md:text-sm text-slate-700 flex items-start gap-1 md:gap-2 leading-tight whitespace-pre-line">
                              <span className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-primary mt-1 md:mt-1.5 shrink-0 whitespace-pre-line"></span> {meal}
                            </li>
                          ))}
                        </ul>
                  ...
                  {Array.from({ length: Math.max(0, 35 - (block.days?.length || 0)) }).map((_, i) => (
                    <div key={"empty-" + i} className="p-2 md:p-4 bg-slate-50/50 min-h-[120px] md:min-h-[220px] whitespace-pre-line"></div>
                  ))}
                </div>
              </div>
              </div>
      ...
      if (block.type === 'menu_features') {
        return (
          <section key={index} className={`py-8 bg-surface-background ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop`} style={getStyle(block, 'container')}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 whitespace-pre-line">
              {block.items?.map((item: any, i: number) => (
                <div key={i} className="bg-white p-6 rounded-xl border border-border-subtle shadow-sm flex items-center gap-4 whitespace-pre-line" style={getCardStyle(item)}>
                  {item.icon && <IconPreview data={item.icon} className="text-4xl whitespace-pre-line" style={{ color: item.iconColor || 'var(--color-primary)'  }} />}
                  <div>
                    <h4 className="font-bold text-slate-800 whitespace-pre-line" style={{ color: item.itemTitleColor }}>{item.title}</h4>
                    <p className="text-sm text-slate-500 whitespace-pre-line" style={{ color: item.itemDescColor }}>{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      if (block.type === 'academic_calendar_hero') {
        return (
          <section key={index} className="relative bg-primary text-on-primary overflow-hidden whitespace-pre-line" style={getStyle(block, 'container')}>
            <div className="absolute inset-0 z-0 whitespace-pre-line">
              <div 
            ...
            <div className={`relative z-10 ${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop py-section-gap flex flex-col items-center ${block.styles?.textAlign ? "" : "text-center"}`}>
              <h1 className="font-display-lg text-display-lg text-on-primary mb-6 whitespace-pre-line" style={getTitleStyle(block)}>{block.title}</h1>
              {block.subtitle && (
                <p className="font-body-lg text-body-lg text-on-primary-container max-w-2xl whitespace-pre-line" style={getSubtitleStyle(block)}>
                  {block.subtitle}
                </p>
              )}
            </div>
      ...
      if (block.type === 'academic_calendar') {
        return (
          <div key={index} className={`${block.styles?.fullWidth ? "max-w-full px-0" : "max-w-container-max"} mx-auto px-margin-desktop py-section-gap`} style={getStyle(block, 'container')}>
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-surface-card p-6 rounded-xl border border-border-subtle shadow-sm whitespace-pre-line">
              <div className="flex items-center space-x-6 mb-4 md:mb-0 whitespace-pre-line">
                ...
                <h2 className="font-headline-xl text-headline-xl text-on-surface whitespace-pre-line">{block.month || "Ekim 2023"}</h2>
                <button className="p-2 rounded-full hover:bg-surface-container-low transition-colors text-on-surface flex items-center justify-center whitespace-pre-line">
                  <span className="material-symbols-outlined whitespace-pre-line" translate="no" aria-hidden="true">chevron_right</span>
                </button>
              </div>
              <div className="flex space-x-4 whitespace-pre-line">
                {block.pdfUrl && (
                  <SmartLink url={block.pdfUrl} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 border-2 border-primary text-primary px-4 py-2 rounded-lg font-label-md text-label-md hover:bg-primary/5 transition-colors whitespace-pre-line">
                    <span className="material-symbols-outlined text-xl whitespace-pre-line" translate="no" aria-hidden="true">picture_as_pdf</span>
            ...
            <div className="bg-surface-card border border-border-subtle rounded-xl overflow-hidden shadow-sm mb-section-gap overflow-x-auto whitespace-pre-line">
              ...
              <div className="grid grid-cols-7 border-b border-border-subtle bg-surface-container-low whitespace-pre-line">
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant hidden md:block`}>Pazartesi</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant hidden md:block`}>Salı</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant hidden md:block`}>Çarşamba</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant hidden md:block`}>Perşembe</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant hidden md:block`}>Cuma</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant text-error hidden md:block`}>Cumartesi</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant text-error hidden md:block`}>Pazar</div>
                
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant md:hidden`}>Pzt</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant md:hidden`}>Sal</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant md:hidden`}>Çar</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant md:hidden`}>Per</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant md:hidden`}>Cum</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant text-error md:hidden`}>Cmt</div>
                <div className={`py-4 ${block.styles?.textAlign ? "" : "text-center"} font-label-md text-label-md text-on-surface-variant text-error md:hidden`}>Paz</div>
              </div>
              ...
              <div className="grid grid-cols-7 border-l border-t border-border-subtle whitespace-pre-line">
                {block.days?.map((day: any, i: number) => {
                  if (!day.isCurrentMonth) {
                    return (
                      <div key={i} className="min-h-[80px] md:min-h-[120px] p-1 md:p-2 border-r border-b border-border-subtle bg-surface-container relative whitespace-pre-line">
                        <span className="font-label-md text-label-md text-on-surface-variant opacity-50 absolute top-1 md:top-2 right-1 md:right-2 whitespace-pre-line">{day.date}</span>
                      </div>
                    );
                  }
                  ...
                  if (day.isWeekend) {
                    return (
                      <div key={i} className="min-h-[80px] md:min-h-[120px] p-1 md:p-2 border-r border-b border-border-subtle bg-surface-container-high relative whitespace-pre-line">
                        <span className="font-label-md text-label-md text-on-surface-variant opacity-50 absolute top-1 md:top-2 right-1 md:right-2 whitespace-pre-line">{day.date}</span>
                        <div className={`mt-8 ${block.styles?.textAlign ? "" : "text-center"} text-on-surface-variant font-caption text-[8px] md:text-caption opacity-50`}>Hafta Sonu</div>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div key={i} className={`min-h-[80px] md:min-h-[120px] p-1 md:p-2 border-r border-b border-border-subtle relative group hover:bg-surface-container-lowest transition-colors ${day.isToday ? 'ring-2 ring-primary ring-inset' : ''} ${day.bgColor ? day.bgColor : 'bg-surface-card'}`}>
                      {day.isToday && <span className="absolute top-1 md:top-2 left-1 md:left-2 bg-primary text-on-primary font-caption text-[8px] md:text-caption px-1.5 py-0.5 rounded uppercase tracking-wider whitespace-pre-line">Bugün</span>}
                      ...
                      <div className="mt-6 md:mt-8 space-y-1 whitespace-pre-line">
                        {day.events && day.events.length > 0 ? day.events.map((event: any, eIndex: number) => (
                          <div key={eIndex} className={`px-1.5 md:px-2 py-0.5 md:py-1 rounded text-[8px] md:text-[10px] font-bold leading-tight truncate border ${event.colorClass || "bg-surface-container text-on-surface border-border-subtle"}`}>
                            {event.title}
                            {event.subtitle && <><br /><span className={`font-normal opacity-90 ${block.styles?.textAlign ? "" : "text-center"} block`}>{event.subtitle}</span></>}
                          </div>
                        )) : day.eventTitle ? (
                          <div className={`px-1.5 md:px-2 py-0.5 md:py-1 rounded text-[8px] md:text-[10px] font-bold leading-tight truncate border ${day.eventColorClass || "bg-surface-container text-on-surface border-border-subtle"}`}>
                            {day.eventTitle}
                            {day.eventSubtitle && <><br /><span className={`font-normal opacity-90 ${block.styles?.textAlign ? "" : "text-center"} block`}>{day.eventSubtitle}</span></>}
                          </div>
                        ) : null}
                      </div>
                    </div>
            ...
            {block.legends && block.legends.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter whitespace-pre-line">
                {block.legends.map((legend: any, i: number) => (
                  <div key={i} className="bg-surface-card border border-border-subtle rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow whitespace-pre-line" style={getCardStyle(legend)}>
                    <div className="flex items-center space-x-3 mb-4 whitespace-pre-line">
                      ...
                      <h3 className="font-headline-md text-headline-md text-on-surface whitespace-pre-line" style={{ color: legend.itemTitleColor }}>{legend.title}</h3>
                    </div>
                    <p className="font-body-md text-body-md text-on-surface-variant mb-4 whitespace-pre-line" style={{ color: legend.itemDescColor }}>{legend.desc}</p>
                    {legend.url && !legend.hideButton && (
                      <SmartLink url={legend.url} className="text-primary font-label-md text-label-md font-bold flex items-center space-x-1 hover:underline whitespace-pre-line">
                        <span>{legend.buttonText || "İncele"}</span>
                        <span className="material-symbols-outlined text-sm whitespace-pre-line" translate="no" aria-hidden="true">arrow_forward</span>
                      </SmartLink>
                    )}
                  </div>
                ))}
