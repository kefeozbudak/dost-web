      ...
      <div className="max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 items-center relative z-10">
        <div className="space-y-6">
          ...
          <h1 className="text-4xl md:text-[48px] font-extrabold text-[#1a1b23] leading-[1.2] tracking-tight" style={getTitleStyle(block)}>
            ...
            {block.title2 && <span className="text-[#1d4eca] block mt-2" dangerouslySetInnerHTML={{ __html: block.title2 }} />}
          </h1>
          {block.subtitle && (
      ...
      <div className="max-w-[1280px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-auto md:auto-rows-[250px]">
          {items.map((item: any, idx: number) => {
            ...
            if (isPrimary) {
              return (
                <div key={idx} className="md:col-span-2 bg-[#1d4eca] text-white rounded-xl shadow-sm p-8 flex flex-col justify-between relative overflow-hidden group">
                  <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                      <div className="text-[#61f9e9] text-[40px] mb-4">
                        <IconPreview data={item.icon || 'forest'} />
                      </div>
                      <h3 className="text-[24px] font-bold text-white mb-2 leading-[1.4]">{item.title}</h3>
                      <p className="text-[#b6c4ff] max-w-md">{item.desc}</p>
                    </div>
                    <a href={item.url || "#"} className="inline-flex items-center gap-2 text-sm font-bold mt-4 hover:text-[#61f9e9] transition-colors w-fit cursor-pointer">
                        {item.buttonText || "Kampüsümüzü İnceleyin"} <span className="material-symbols-outlined text-[18px]">arrow_forward</span>
                    </a>
                  </div>
                  <div className="absolute right-0 top-0 w-full h-full bg-gradient-to-l from-[#003bb0]/20 to-transparent z-0"></div>
                </div>
              );
            }
            ...
            if (isLarge) {
              return (
                <div key={idx} className="md:col-span-2 bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-8 flex flex-col justify-between group hover:shadow-md transition-shadow relative overflow-hidden">
                  <div className="relative z-10">
                    <div className="text-[#1d4eca] text-[40px] mb-4">
                      <IconPreview data={item.icon || 'psychology'} />
                    </div>
                    <h3 className="text-[24px] font-bold text-[#1a1b23] mb-2 leading-[1.4]">{item.title}</h3>
                    <p className="text-[#434654] max-w-md">{item.desc}</p>
                  </div>
            ...
            return (
              <div key={idx} className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-8 flex flex-col justify-between hover:bg-[#f3f2fd] transition-colors group">
                <div>
                  <div className="text-[#006a62] text-[32px] mb-4">
                    <IconPreview data={item.icon || 'favorite'} />
                  </div>
                  <h3 className="text-[14px] font-bold text-[#1a1b23] mb-2">{item.title}</h3>
      ...
      <div className="max-w-[1280px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {items.map((item: any, idx: number) => {
             ...
             return (
              <div key={idx} className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
          ...
          <div className="space-y-4">
             {items.map((item: any, idx: number) => (
               <div key={idx} className="flex gap-4">
                 <div className="text-[#1d4eca] text-2xl shrink-0">
        ...
        <div className="z-10 flex flex-col gap-6">
          ...
          <h1 className="text-[48px] font-[800] text-[#0f172a] leading-[1.2] tracking-tight" style={getTitleStyle(block)}>
            ...
            {block.title2 && (
              <>
                <br />
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#1d4eca] to-[#006a62]" dangerouslySetInnerHTML={{ __html: block.title2 }}></span>
              </>
            )}
          </h1>
          {block.subtitle && (
