...
export interface PopupData {
  ...
  description: string;
  imageUrl?: string;
  imagePosition?: 'left' | 'top' | 'right' | 'bg';
  buttonText?: string;
  buttonUrl?: string;
  onlyImage?: boolean;
      ...
      >
        ...
        {currentPopup.pageEmbed ? (
          ...
          <div className={`flex flex-col ${imgPos === 'left' ? 'md:flex-row' : imgPos === 'right' ? 'md:flex-row-reverse' : ''}`}>
            {/* Image Section */}
            {currentPopup.imageUrl && (
              <div className={`w-full ${imgPos === 'left' || imgPos === 'right' ? 'md:w-1/2 min-h-[260px] md:min-h-[320px]' : 'h-48'} relative bg-slate-100 overflow-hidden`}>
                <img
                  src={currentPopup.imageUrl}
            ...
            {/* Content Section */}
            <div className={`w-full ${currentPopup.imageUrl && (imgPos === 'left' || imgPos === 'right') ? 'md:w-1/2' : ''} p-6 sm:p-8 flex flex-col justify-center text-left`}>
              ...
              <p className="opacity-80 text-sm leading-relaxed mb-6 whitespace-pre-line">
