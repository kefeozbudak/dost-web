...
// The second if (day.isToday) block is from line 6356 to 6407 approximately.
// Let's use a substring or index to remove it.
const searchStr = `                      if (day.isToday) {
                        return (
                          <div
                            key={i}
...
let secondIndex = content.indexOf(searchStr, firstIndex + 10);

if (secondIndex !== -1) {
    content = content.substring(0, secondIndex) + content.substring(secondIndex + searchStr.length);
    console.log("Removed duplicate isToday in menu_calendar");
} else {
    // maybe there's a difference in \n vs actual newline
    console.log("Not found exact duplicate. Doing regex.");
    let arr = content.split("                      if (day.isToday) {");
    if (arr.length > 2) {
       // manually removing the second one in the menu_calendar area
    }
}
